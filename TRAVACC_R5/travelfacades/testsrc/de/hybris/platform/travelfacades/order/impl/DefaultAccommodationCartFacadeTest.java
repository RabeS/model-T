...
public class DefaultAccommodationCartFacadeTest
{
	...
	public void testAddProductsToCartWithOrderEntries()
	{
		try
		{
			...
			final Date endingDate = TravelDateUtils.addDays(startingDate, 1);
			given(accommodationOrderEntryGroupModel.getStartingDate()).willReturn(startingDate);
			given(accommodationOrderEntryGroupModel.getEndingDate()).willReturn(endingDate);
	...
	public void testAddAccommodationBundleToCart() throws CommerceCartModificationException
	{
		...
		given(travelBundleCommerceCartFacade.startBundle(Matchers.anyString(), Matchers.anyString(), Matchers.anyLong(),
				Matchers.any()))
				.willReturn(cartModification);
		given(travelBundleCommerceCartFacade.addToCart(Matchers.anyString(), Matchers.anyLong(), Matchers.anyInt())).willReturn(cartModification);
