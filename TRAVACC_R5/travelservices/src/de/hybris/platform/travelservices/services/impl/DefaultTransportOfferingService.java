...
public class DefaultTransportOfferingService implements TransportOfferingService
{
	...
	{
		return orderEntryList.stream().flatMap(entry -> entry.getTravelOrderEntryInfo().getTransportOfferings().stream()).distinct()
				.collect(Collectors.toList());
	}

	@Override
	public List<TransportOfferingModel> createTransportOfferingForScheduleConfiguration(
			...
			final LocalDate date)
	{
		for (final ScheduleConfigurationDayModel scheduleConfigurationDay : scheduleConfiguration.getScheduleConfigurationDays())
		{
			if (scheduleConfigurationDay.getDayOfWeek().ordinal() == date.getDayOfWeek().getValue() % 7)
			{
				...
				populateTransportOffering(scheduleConfigurationDay, transportOffering, scheduleConfiguration, date);
				if (validateUniqueTransportOfferingCode(transportOffering.getCode()))
				{
	 ...
	 * @return
	 */
	protected boolean validateUniqueTransportOfferingCode(final String code)
	{
		Boolean result = Boolean.FALSE;
		try
		{
			getTransportOfferingDao().findTransportOffering(code);
		}
		catch (final ModelNotFoundException ex)
		{
			LOG.debug(ex);
	 ...
	 * 		the date
	 */
	protected void populateTransportOffering(final ScheduleConfigurationDayModel scheduleConfigurationDay,
			...
			final LocalDate date)
	{
		...
		transportOffering.setScheduleConfiguration(scheduleConfiguration);
	}
			...
			final ScheduleConfigurationDayModel scheduleConfigurationDay, final TransportOfferingModel transportOffering)
	{
		...
		final ZonedDateTime originUtcDateTime = TravelDateUtils.getUtcZonedDateTime(transportOffering.getDepartureTime(),
				originZoneId);
		final ZonedDateTime destinationUtcDateTime = TravelDateUtils.getUtcZonedDateTime(transportOffering.getDepartureTime(),
				destinationZoneId);

		final Duration duration = Duration.between(destinationUtcDateTime, originUtcDateTime);

		final Calendar arrivalCalendar = Calendar.getInstance();
		arrivalCalendar.setTime(transportOffering.getDepartureTime());

		arrivalCalendar.add(Calendar.HOUR,
				Objects.nonNull(scheduleConfigurationDay.getDurationHrs()) ? scheduleConfigurationDay.getDurationHrs() : 0);
		arrivalCalendar.add(Calendar.MINUTE,
				Objects.nonNull(scheduleConfigurationDay.getDurationMins()) ? scheduleConfigurationDay.getDurationMins() : 0);

		arrivalCalendar.add(Calendar.SECOND, (int) duration.getSeconds());
	...
	@Override
	public SearchResult<TransportOfferingModel> getTransportOfferings(final int batchSize, final int offset)
	{
		ServicesUtil.validateParameterNotNull(batchSize, "batchSize Number can not be null");
		ServicesUtil.validateParameterNotNull(offset, "offset can not be null");
		return getTransportOfferingDao().findTransportOfferings(batchSize, offset);
	}
}
