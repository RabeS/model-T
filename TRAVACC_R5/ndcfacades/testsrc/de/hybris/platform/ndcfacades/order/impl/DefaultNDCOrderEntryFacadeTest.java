...
import de.hybris.platform.ndcfacades.ndc.OrderCreateRQ.Query.OrderItems;
...
public class DefaultNDCOrderEntryFacadeTest
{
	...
	public void testCreateOrderEntriesWithConversionException() throws CalculationException, NDCOrderException
	{
		...
		final List<Passenger> passengerList = orderCreateRQ.getQuery().getPassengers().getPassenger();
		...
		passenger.setObjectKey("objectKey");
		final PTC ptc=new PTC();
		ptc.setValue("ptcValue");
		...
		passengersForOfferItem.getPassengerReference().add(passenger);
		...
		Mockito.when(travellerService.createTraveller(TravellerType.PASSENGER.getCode(),
				passengerType.getCode(), passenger.getObjectKey(), 0, "", order.getCode())).thenReturn(travellerModel);
