...
public class AncillaryPageController extends AbstractCheckinStepController
{
	...
	@Resource(name = "offerTypeMap")
	private Map<String, String> offerTypeMap;

	@Resource(name = "offerTypeMapForGroups")
	private Map<String, String> offerTypeMapForGroups;
	...
	protected String getAncillaryPage(final Model model, final String ancillaryCmsPage) throws CMSItemNotFoundException
	{
				...
				.getInt(TravelfacadesConstants.GROUP_BOOKING_THRESHOLD))
		{
			offerResponseData.getOfferGroups().stream()
					.filter(offerGroup -> StringUtils.isNotEmpty(offerGroup.getOfferType()))
					.forEach(offerGroup -> offerGroupsViewMap
							.put(offerGroup.getCategoryType(), offerTypeMap.get(offerGroup.getOfferType())));
		...
		else
		{
			offerResponseData.getOfferGroups().stream()
					.filter(offerGroup -> StringUtils.isNotEmpty(offerGroup.getOfferType()))
					.forEach(offerGroup -> offerGroupsViewMapForGroupBooking
							.put(offerGroup.getCategoryType(), offerTypeMapForGroups.get(offerGroup.getOfferType())));
