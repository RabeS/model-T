...
import de.hybris.platform.ndcfacades.ndc.OrderChangeRQ.Query.Passengers.Passenger.ActionType;
...
public class NDCOrderChangeOfferValidator implements NDCRequestValidator<OrderChangeRQ>
{
	...
	public void validate(final OrderChangeRQ orderChangeRQ, final ErrorsType errorsType)
	{
		...
		try
		{
			if (!validateOrderItemsIdPerPTC(orderItems, errorsType))
			{
				return;
			}

			validateOrderItemAssociations(orderChangeRQ.getQuery().getOrder().getActionType().getValue(), orderItems, errorsType);
		}
			...
			final ErrorsType errorsType)
	{
		...
		final NDCActionType ndcActionType = getEnumerationService().getEnumerationValue(NDCActionType.class,
		...
		for (final OrderItem orderItem : orderItems)
		{
			...
			if ((NDCActionType.ADD_ANCILLARIES.equals(ndcActionType) || NDCActionType.REMOVE_ANCILLARIES.equals(ndcActionType))
					&& Objects.isNull(orderItem.getAssociations().getServices()))
			{
				addError(errorsType,
			...
			throws NDCOrderException
	{
		for (final OrderItem orderItem : orderItems)
		{
			...
			if (Objects.isNull(orderItem.getAssociations()) || Objects.isNull(orderItem.getAssociations().getPassengers())
					|| CollectionUtils.isEmpty(orderItem.getAssociations().getPassengers().getPassengerReferences()))
			{
				addError(errorsType,
						getConfigurationService().getConfiguration().getString(NdcwebservicesConstants.MISSING_PASSENGER_REFERENCE));
				return false;
			...
			for (final Object passengerObj : orderItem.getAssociations().getPassengers().getPassengerReferences())
			{
