...
public class DefaultAccommodationCartFacade extends DefaultCartFacade implements AccommodationCartFacade
{
			...
			throws CommerceCartModificationException
	{
		...
		for (int i = 0; i < numberOfRooms; i++)
		{
			...
			getAccommodationCommerceCartService()
					.createOrderEntryGroup(entryNumbers, checkInDate, checkOutDate, accommodationOfferingCode, accommodationCode,
							ratePlanCode, newRefNumber + i);
		}
		/*
		 *	Recalculating cart after EntryGroup for selected accommodation is created.
		 *	This is done for promotions that needs EntryGroup info.
		 */
			...
			throws CommerceCartModificationException
	{
		...
		{
			addAccommodationToCart(checkInDate, checkOutDate, accommodationOfferingCode, accommodationCode, rates,
					roomQuantityToAdd,
					ratePlanCode);
		}
		...
		{
			getAccommodationCommerceCartService()
					.removeAccommodationOrderEntryGroups(accommodationOrderEntryGroups, Math.abs(roomQuantityToAdd));
		}
	}
			...
			final int numberOfRooms, final String ratePlanCode, final String paymentType) throws CommerceCartModificationException
	{
		for (int i = 0; i < numberOfRooms; i++)
		{
			...
			final boolean isSuccess = getAccommodationCommerceCartService()
					.amendOrderEntryGroup(accommodationOrderEntryGroup, entryNumbers, getAccommodationCommerceCartService()
							.getPaymentTypeForChangeDates(accommodationOrderEntryGroup, entryNumbers));
			if (!isSuccess)
			{
	...
	{
		getAccommodationCommerceCartService()
				.rollbackAccommodationEntries(accommodationCode, accommodationOfferingCode, ratePlanCode);
	}
	...
	{
		getAccommodationCommerceCartService()
				.cleanupCartBeforeAddition(accommodationOfferingCode, checkInDateTime, checkOutDateTime);
	}
	...
	/**
	 * Overriding the method to force creation of new entry. This is because we need a separate entry of the same product to
	 * represent a different accommodation order entry group.
	 *
	 * @param code
	 * @param quantity
	 * @return
	...
	public CartModificationData addToCart(final String code, final long quantity) throws CommerceCartModificationException
	{
		...
		parameter.setOrderEntryType(OrderEntryType.ACCOMMODATION);
			...
			throws CommerceCartModificationException
	{
		...
		final long updatedQuantity = quantity * TravelDateUtils
				.getDaysBetweenDates(accommodationOrderEntryGroupModel.getStartingDate(),
						accommodationOrderEntryGroupModel.getEndingDate());
		...
		if (!existingEntryOptional.isPresent())
		{
			...
			//createNewOrderEntry
			final CartModificationData cartModificationData = addToCart(productCode, updatedQuantity);

			final AbstractOrderEntryModel newOrderEntry = getCartService()
					.getEntryForNumber(getCartService().getSessionCart(), cartModificationData.getEntry().getEntryNumber());
		...
		else
		{
			...
			try
			{
				getCartFacade().updateCartEntry(existingOrderEntry.getEntryNumber(), updatedQuantity);
			...
			throws CommerceCartModificationException
	{
		...
		for (final RoomRateCartData roomRate : roomRates)
		{
			final CartModificationData cartModification = CollectionUtils.isEmpty(cartModificationDatas) ?
					getTravelBundleCommerceCartFacade()
							.startBundle(accommodationBundleTemplateModel.getId(), roomRate.getCode(), roomRate.getCardinality(),
									OrderEntryType.ACCOMMODATION) :
					getTravelBundleCommerceCartFacade().addToCart(roomRate.getCode(), roomRate.getCardinality().longValue(),
		...
		getAccommodationCommerceCartService().createOrderEntryGroup(
				cartModificationDatas.stream().map(cartModification -> cartModification.getEntry().getEntryNumber())
						.collect(Collectors.toList()), addDealToCartData.getStartingDate(), addDealToCartData.getEndingDate(),
	...
	@Override
	public void replaceAccommodationInCart(final Date checkInDate, final Date checkOutDate, final String
			...
			final Integer roomStayRefNumber) throws CommerceCartModificationException
	{
		...
		try
		{
			...
			for (int i = 0; i < numberOfRooms; i++)
			{
				...
				getAccommodationCommerceCartService()
						.createOrderEntryGroup(entryNumbers, checkInDate, checkOutDate, accommodationOfferingCode, accommodationCode,
								ratePlanCode, roomStayRefNumber);
			}
		}
	 ...
	 *
	 * @param accommodationBundleTemplate
	 * 		the accommodation bundle template
	 * @param addDealToCartData
	 * 		the add deal to cart data
	 * @return the list
	 */
	...
	protected boolean validateRoomRateAgainstDate(final Date date, final ProductModel roomRate)
	{
		...
		{
			if (!date.before(dateRange.getStartingDate()) && !date.after(dateRange.getEndingDate()) && isValidDayOfWeek(date,
	 ...
	 * @param date
	 * 		the date
	 * @return the room rate cart data
	 */
	...
	{
		if (StringUtils.isEmpty(accommodationUid) || StringUtils.isEmpty(transportOfferingCode) || StringUtils
				.isEmpty(travellerCode) || StringUtils.isEmpty(originDestinationRefNo) || StringUtils.isEmpty(travelRoute))
		{
			return Boolean.FALSE;
		}
		...
		{
			final AddToCartResponseData response = strategy
					.validateSelectedAccommodation(accommodationUid, StringUtils.EMPTY, transportOfferingCode, travellerCode,
							originDestinationRefNo, travelRoute);
		...
		if (getAccommodationMapService().isSeatInCart(cart, accommodation, transportOfferingCode) || !getAccommodationMapService()
		...
		if (Objects.nonNull(associatedProduct) && (associatedProduct instanceof AccommodationModel || ProductType.ACCOMMODATION
				.equals(associatedProduct.getProductType())))
		{
			final String productCode = associatedProduct.getCode();
			final OrderEntryData existingOrderEntry = getCartFacade()
					.getOrderEntry(productCode, travelRoute, transportOfferingCodes, travellerCode, false);
			if (Objects.isNull(existingOrderEntry))
			{
				...
				try
				{
					final PriceLevel priceLevel = getTravelCommercePriceFacade()
							.getPriceLevelInfo(productCode, transportOfferingCodes, travelRoute);
					if (Objects.isNull(priceLevel))
					{
						return Boolean.FALSE;
					}
					getTravelCommercePriceFacade()
							.setPriceAndTaxSearchCriteriaInContext(priceLevel, transportOfferingCodes, traveller);
					...
					if (cartModification.getQuantityAdded() >= PRODUCT_QUANTITY)
					{
						...
						//Cart recalculation is needed after persisting pricelevel, traveller details, travel route and transport
						// offering codes against cart entry
						//to make "pricing search criteria in context" logic work. Reason is that, OOTB cart calculation gets
						// triggered while creation of a cart entry,
						//and by that time due to missing travel specific details (pricelevel, traveller details, travel route and
						// transport offering codes), incorrect
			...
			else if (existingOrderEntry.getQuantity() == 0L)
			{
				try
				{
					...
					{
						LOG.debug(
								"Product Code:" + cartModification.getEntry().getProduct().getCode() + "has been update with Quantity:"
	...
	{
		if (StringUtils.isEmpty(accommodationUid) || StringUtils.isEmpty(transportOfferingCode) || StringUtils
				.isEmpty(travellerCode) || StringUtils.isEmpty(travelRoute))
		{
			return Boolean.FALSE;
		}
		...
		{
			final AddToCartResponseData response = strategy
					.validateSelectedAccommodation(accommodationUid, null, transportOfferingCode, travellerCode, null, travelRoute);
	 ...
	 *
	 * @return TravellerFacade traveller facade
	 */
	 ...
	 */
	@Required
	public void setTravelBundleCommerceCartFacade(final TravelBundleCommerceCartFacade travelBundleCommerceCartFacade)
	{
