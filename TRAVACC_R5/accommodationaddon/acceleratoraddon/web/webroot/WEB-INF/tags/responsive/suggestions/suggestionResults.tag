...
<c:choose>
	<c:when test="${not empty suggestionResult}">
...
<c:if test="${empty suggestionResult && isRestClientException=='true' }">
	<div class="autocomplete-suggestions no-matching-airports">
		<ul>
			<li>
				<spring:theme code="accommodation.booking.details.googlemap.autosuggest.unavailable" />
			</li>
		</ul>
	</div>
</c:if>
