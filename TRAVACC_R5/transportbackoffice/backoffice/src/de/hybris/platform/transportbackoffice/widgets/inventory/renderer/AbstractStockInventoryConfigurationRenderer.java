...
import de.hybris.platform.travelbackofficeservices.model.travel.ManageAncillaryInventoryModel;
...
public abstract class AbstractStockInventoryConfigurationRenderer extends DefaultCustomViewRenderer
{
	...
	private static final String ANCILLARY_LABEL = "create.inventory.ancillary.assigninventory.wizard.ancillary";
			...
			final ManageStockLevelInfo manageStockLevel)
	{
		...
		if (Objects.isNull(widgetInstanceManager.getModel().getValue("ancillaryInventoryModel",ManageAncillaryInventoryModel.class)))
		{
			productLabel = createLabel(Labels.getLabel(BOOKING_CLASS_LABEL) + TravelbackofficeConstants.COLON, true);
		}
		else
		{
			productLabel = createLabel(Labels.getLabel(ANCILLARY_LABEL) + TravelbackofficeConstants.COLON, true);
		}
		...
		addButtonEventListener = event -> {
			if (StringUtils.equals(Events.ON_CLICK, event.getName()))
			{
				stockLevelGrid.getRows().appendChild(createStockAttributesColumnLayout(widgetInstanceManager,
						Integer.parseInt(event.getTarget().getId()) + 1, addButtonEventListener, removeButtonEventListener, null, Boolean.TRUE));
				container.appendChild(stockLevelGrid);
		...
		if (CollectionUtils.isNotEmpty(manageStockLevel.getStockLevelAttributes()) && Objects.nonNull(fromPreviewStep))
		{
			for(int count = 1; count <= manageStockLevel.getStockLevelAttributes().size(); count ++)
			{
				...
				stockLevelGrid.getRows().appendChild(createStockAttributesColumnLayout(
						widgetInstanceManager, count, addButtonEventListener, removeButtonEventListener, manageStockLevel.getStockLevelAttributes().get(count - 1), isPlusButtonNeedToAdd));
			}
		}
		else
		{
	 ...
	 * 		true if plus button need to add otherwise false
	 * @return row
	 */
	...
	protected Editor createEditor(final WidgetInstanceManager widgetInstanceManager, final String type,
			final Object initialValue, final boolean nestedObjectCreation, final String property)
	{
		final Editor editor = new Editor();
