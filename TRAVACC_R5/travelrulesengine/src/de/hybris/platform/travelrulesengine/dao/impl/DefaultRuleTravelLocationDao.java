...
public class DefaultRuleTravelLocationDao extends DefaultGenericDao<LocationModel> implements RuleTravelLocationDao
{
	...
	@Override
	public List<LocationModel> findLocationsByName(final String name)
	{
		validateParameterNotNull(name, "Location name must not be null!");

		final List<LocationModel> locationModels = find(Collections.singletonMap(LocationModel.NAME, (Object) name));
		if (CollectionUtils.isEmpty(locationModels))
		{
			LOG.info("No result for the given query");
			return null;
