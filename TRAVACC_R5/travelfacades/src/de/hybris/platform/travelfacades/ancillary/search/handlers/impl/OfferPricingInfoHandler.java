...
public class OfferPricingInfoHandler implements AncillarySearchHandler
{
	 ...
	 * Collect common products across all transportOfferings, whose product code matches with the products from above list and
	 * stock level is more than zero.
	...
	protected void showProducts(final OfferRequestData offerRequestData, final OfferResponseData offerResponseData)
	{
		...
		{
			LOGGER.debug("No offerRequest Data: " + offerRequestData + "provided", e);
		}
	}
		...
		for (final OfferGroupData offerGroupData : offerGroups)
		{
			...
			for (final OriginDestinationOfferInfoData originDestinationOfferInfo : offerGroupData.getOriginDestinationOfferInfos())
			{
				final List<OfferPricingInfoData> pricedOffersToRemove = originDestinationOfferInfo.getOfferPricingInfos().stream()
						.filter(offerPricingInfo -> BooleanUtils.isNotTrue(offerPricingInfo.getProduct().isIgnoreRules())
	 ...
	 * 		the products for category
	 * @param originDestinationOfferInfo
	 * 		the origin destination offer info
	 */
	...
	/**
	 * This method collects the products that belong to a category(offerGroupData.getCode()), common across all transportOfferings
	 * and stockLevel more than zero.
	 *
	 * @param productsForCategory
	...
	/**
	 * This method collects the common products across transportOfferings in case of multisectors or returns the available products
	 * if single sector.
	 *
	 * @param productsPerTransportOffering
	 * 		the products per transport offering
	...
	/**
	 * This method returns available products for each transportOffering. The product in offer should be of category of products as
	 * retrieved using the category code(offerGroupData.getCode()) and the available stockLevel of the product should be more than
	 * zero.
	 *
	 * @param productsForCategory
	 * 		the products for category
	 * @param originDestinationOfferInfo
	 * 		the origin destination offer info
	 * @return available products for transport offering
	 */
	 ...
	 * @return the available products in offer
	 */
