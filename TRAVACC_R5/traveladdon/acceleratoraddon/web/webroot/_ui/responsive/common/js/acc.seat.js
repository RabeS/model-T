ACC.seat = {
	...
	init: function () {
		...
		$("#carousel-generic").owlCarousel({
			...
			rewindSpeed: 0,
			mouseDrag: false,
			afterAction: positionControls,
		});

		function positionControls(){
			var headerHeight = $('#carousel-generic .modal-header').height(),
					paddingTop = parseInt($('#carousel-generic .modal-header').css('padding-top')),
					total = headerHeight + paddingTop;

			$('#carousel-generic .owl-pagination').css('top', total + 'px');
		}
	},
	...
	bindSelectSeatButton: function () {
		...
		$('#y_selectSeat').on("click", function () {
			$('#y_selectSeat').addClass("disabled");
	...
	disableButtonsOnPage: function(){
		$(".y_reservationContinueButton").addClass('disabled');
		$(".y_reservationContinueButton").unbind('click');
	...
	enableButtonsOnPage: function(){
		$(".y_reservationContinueButton").removeClass('disabled');
		$(".y_reservationContinueButton").bind('click');
	...
	initializeJsBinding: function () {
		...
		ACC.seat.setSeatIconToInitials();
	},

	bindActiveSeatmapSelectSeat: function () {
		var activeSeatmaps = $(document).find('.active .y_transportOffering.item').find(".y_seatMapSVGWrap");
	...
	bindDesktopHoverInfo: function () {
		$(document).on('mouseenter', '.seat', function () {
			$(document).find('.active .y_transportOffering.item').find(".seatInfo-Box").show();
		});

		$(document).on('mouseleave', '.seat', function () {
			$(document).find('.active .y_transportOffering.item').find(".seatInfo-Box").hide();
		...
		$(document).on('mousemove', '.seat', function (e) {
			...
			var seatInfoBox = $(document).find('.active .y_transportOffering.item').find(".seatInfo-Box");
			var seatInfoBoxContainer = $(document).find('.active .y_transportOffering.item').find('.seatmapSVG');
			...
			var findPassengerID = $(document).find(".y_svgdiv").find(".active div.item").find(".y_seatIcon.active").parent().attr('id');
	...
	populateSelectedSeats: function (onlyActiveSeatmap) {
		...
		if (onlyActiveSeatmap === "true") {
			seatMapTabs = $(document).find('.active .y_transportOffering.item');
		} else {
	...
	setPassengerSeatFromCart: function (seatMapIndex, selectedSeat) {
		...
		passengerDivs.each(function () {
			if ($(this).data('passengertype') == selectedSeat.traveller.label &&
				...
				var activeSeatmap = $(document).find('.active .y_transportOffering.item').find(".y_seatMapSVGWrap").parent();
	...
	setSeatIconToInitials: function () {
		if (ACC.seat.isAmend === "true") {
			var seatIcons = $(document).find('.y_seatIcon');

			for (var j = 0; j < seatIcons.length; j++) {
				var passengerNameDiv = $(seatIcons[j]).find('.y_initials');
				var passengerName = $(passengerNameDiv).text();
				var initial = ACC.seat.getInitials(passengerName);
				passengerNameDiv.text(initial);
			}
		}
	},

	getInitials: function (passengerName) {
		var sections = passengerName.split(" ");
		var initials = "";

		for (var i = 0; i < sections.length; i++) {
			if (sections[i].length > 0 && sections[i] !== '') {
				initials += sections[i][0]
			}
		}
		return initials;
	},
	...
	disableUnavailableSeat: function () {
		var activeItem = $(document).find(".y_svgdiv").find(".active div.item");
	...
	setLegendOption: function () {
		var iconClass;
		var activeItem = $(document).find('.owl-item.active')
		var activeSeatmap = activeItem.find(".y_seatMapSVGWrap").parent();
		var seats = activeSeatmap.find(".seat");

		$(seats).each(function () {
			var el = $(this).attr("class");
			if (el.indexOf("unavailable") != -1) {
				return;
			}
			iconClass = $(this).data('cabinclass');
			return false;
		});

		var classToAdd = ACC.addons.travelacceleratorstorefront['ancillary.seatmap.legend.label.' + iconClass];

		activeItem.find(".y_cabinClassIcon").addClass(iconClass);
		if (classToAdd) {
			activeItem.find(".y_cabinClassLabel").text(classToAdd);
		}
	},

	bindDeckChangeButton: function () {
		$(document).on('click', '.y_deckBtn', function () {
			var activeDeckButtons = $(document).find('.active .y_transportOffering.item').find(".y_deckBtn");
	...
	bindPassengerIconChange: function () {
		$(document).on('click', '.y_seatIcon', function () {
			...
			var seats = $(activeSeatmap).find(".seat");
			var activeItem = $(document).find(".y_svgdiv").find(".active div.item");
			var findPassengerID = $(activeItem).find(".y_seatIcon.active").parent().attr('id');
	...
	bindCarousel: function () {
		$(document).on('touchend click', '.owl-buttons .owl-prev, .owl-buttons .owl-next, .owl-pagination .owl-page', function () {

			$('.owl-next, .owl-prev, .owl-page').addClass("disabled");
			if (!$('.owl-next, .owl-prev, .owl-page').hasClass("disabled")) {
				return;
			}

			else {
				ACC.seat.currentSeatMapIndex = $(".y_svgdiv .owl-item.active div.item").data("tabid");

				var seatMap = ACC.seat.seatMapJsonObj.seatMap[ACC.seat.currentSeatMapIndex];
				var svgWrap = $(".y_svgWrap[data-transportofferingnumber='" + seatMap.transportOffering.code + "']");
				if (svgWrap.find('.y_seatMapSVGWrap').is(":not(:empty)")) {
					$('.owl-next, .owl-prev, .owl-page').removeClass("disabled");
					return;
				}
	...
	desktopSelectSeat: function (selectedSeat) {
		var currentSeats = $(selectedSeat).parents('div.y_svgWrap').find('.seat');
		...
		var passengerID = $(activeItem).find(".y_seatIcon.active").parent().attr('id');
		...
		return $.when(ACC.services.addRemoveSeats(addRemoveAccommodations)).then(
				function (response) {
					...
					if (jsonData.valid) {
						...
						$("#y_addProductToCartErrorWithOKModal").dialog('open');
					}
				});
	},
