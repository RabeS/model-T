...
/**
 * Strategy to add to the specified order a PER_LEG ancillary.
 */
public class AddToOrderPerLegStrategy extends AbstractAddAncillariesToOrderRestrictionStrategy
{
	@Override
			...
			final int originDestinationRefNumber, final List<AbstractOrderEntryModel> orderEntries) throws NDCOrderException
	{
		...
		if (StringUtils.equalsIgnoreCase(mapping, TravelservicesConstants.TRANSPORT_OFFERING))
		{
			...
			{
				getNdcProductFacade().checkIfProductCanBeAddedToTraveller(order, "", ancillaryProduct,
						NdcfacadesConstants.DEFAULT_ANCILLARY_QUANTITY, Collections.singletonList(transportOffering.getCode()),
						routeCode);

				add(order, ancillaryProduct, null, Collections.singletonList(transportOffering), offerItemID, routeCode,
		...
		else if (StringUtils.equalsIgnoreCase(mapping, TravelservicesConstants.TRAVEL_ROUTE))
		{
			...
			getNdcProductFacade()
					.checkIfProductCanBeAddedToTraveller(order, "", ancillaryProduct, NdcfacadesConstants.DEFAULT_ANCILLARY_QUANTITY,
							new LinkedList<>(transportOfferingCodes), routeCode);

			add(order, ancillaryProduct, null, transportOfferings, offerItemID, routeCode, originDestinationRefNumber, orderEntries);
		}
	}
}
