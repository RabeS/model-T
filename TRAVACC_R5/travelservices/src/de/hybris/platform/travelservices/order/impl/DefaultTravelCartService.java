...
public class DefaultTravelCartService extends DefaultCartService implements TravelCartService
{
	...
	{
		final Long quantityInStock = getTravelCommerceStockService().getStockLevelQuantity(productModel,
				Collections.singleton(transportOfferingModel));
			...
			final String returnDate)
	{
		...
		final Optional<AbstractOrderEntryModel> optionalOutboundEntry = cartModel.getEntries().stream()
				.filter(entry -> OrderEntryType.TRANSPORT.equals(entry.getType())
						...
						|| ProductType.FEE.equals(entry.getProduct().getProductType()))
	...
	public BigDecimal getTransportTotalByEntries(final AbstractOrderModel order, final List<AbstractOrderEntryModel> entries)
	{
		...
		final Double globalDiscounts = getGlobalDiscountForEntryType(order,OrderEntryType.TRANSPORT).doubleValue();
		final Double entriesDiscounts = entries.stream()
				.filter(entryModel -> entryModel.getProduct() instanceof FareProductModel
						|| ProductType.FARE_PRODUCT.equals(entryModel.getProduct().getProductType()) || entryModel.getBundleNo() == 0)
	...
	@Override
	public Boolean isNetPricedEntry(final AbstractOrderEntryModel entry)
	{
		return Optional.ofNullable(getTravelCommercePriceService().isNetPricedProduct(entry.getProduct()))
				.orElse(entry.getOrder().getNet());
	}

	@Override
	public BigDecimal getGlobalDiscountForEntryType(final AbstractOrderModel abstractOrder, final OrderEntryType entryType)
	{
		return getBookingService().getGlobalDiscountForEntryType(abstractOrder,entryType);
	}
	...
	@Override
	public BigDecimal getEntriesTotal(final List<AbstractOrderEntryModel> entries)
	{
		if(CollectionUtils.isEmpty(entries))
		{
			return BigDecimal.ZERO;
		}
		final AbstractOrderModel abstractOrder = entries.get(0).getOrder();
		BigDecimal entriesTotal = entries.stream().map(entry -> BigDecimal.valueOf(entry.getBasePrice() * entry.getQuantity()))
		...
		if (abstractOrder.getNet())
		{
			final List<TaxValue> taxValues = entries.stream().flatMap(entry -> entry.getTaxValues().stream())
					.collect(Collectors.toList());
			entriesTotal = entriesTotal.add(CollectionUtils.isNotEmpty(taxValues) ?
					taxValues.stream().map(taxValue -> BigDecimal.valueOf(taxValue.getAppliedValue()))
							.reduce(BigDecimal.ZERO, BigDecimal::add) : BigDecimal.ZERO);
		}

		BigDecimal discount = BigDecimal.ZERO;
		final GlobalDiscountByEntryTypeCalculationStrategy strategy = getGlobalDiscountCalculationStrategyMap()
				.getOrDefault(entries.get(0).getType(), getGlobalDiscountCalculationStrategyMap().get(OrderEntryType.DEFAULT));
		...
		if (discount.doubleValue() > 0 && abstractOrder.getNet())
		{
			...
			entriesTotal = entriesTotal.subtract(discount);
		}
