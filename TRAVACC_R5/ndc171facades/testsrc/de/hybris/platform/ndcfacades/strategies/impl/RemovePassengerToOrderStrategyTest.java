...
public class RemovePassengerToOrderStrategyTest
{
	@InjectMocks
	RemovePassengerToOrderStrategy removePassengerToOrderStrategy;
	...
	public void testAmendOrder() throws NDCOrderException
	{
		...
		Assert.assertNotNull(removePassengerToOrderStrategy.amendOrder(order, testSetup.createOrderChangeRQ()));
	}

	@Test(expected = NDCOrderException.class)
	public void testAmendOrderForException() throws NDCOrderException
	{
		Mockito.when(bookingFacade.atleastOneAdultTravellerRemaining(Matchers.anyString(), Matchers.anyString()))
		...
		Mockito.when(cloneAbstractOrderStrategy.clone(Matchers.anyObject(), Matchers.anyObject(), Matchers.any(OrderModel.class),
				Matchers.anyString(), Matchers.any(Class.class), Matchers.any(Class.class))).thenReturn(order);
		removePassengerToOrderStrategy.amendOrder(order, testSetup.createOrderChangeRQ());
	}

	@Test(expected = NDCOrderException.class)
	public void testAmendOrderForInactiveEntries() throws NDCOrderException
	{
		Mockito.when(bookingFacade.atleastOneAdultTravellerRemaining(Matchers.anyString(), Matchers.anyString()))
				.thenReturn(Boolean.TRUE);
		final OrderModel order = testSetup.createOrderModel(testSetup.createCurrencyModel("EUR"), false);
		Mockito.when(cloneAbstractOrderStrategy.clone(Matchers.anyObject(), Matchers.anyObject(), Matchers.any(OrderModel.class),
				Matchers.anyString(), Matchers.any(Class.class), Matchers.any(Class.class))).thenReturn(order);
		removePassengerToOrderStrategy.amendOrder(order, testSetup.createOrderChangeRQ());
	}
	...
	private class TestSetup
	{
		...
		private PassengerType createPassenger()
		{
			PassengerType passengerType = new PassengerType();
			passengerType.setProfileID("TEST_UID");
		...
		public OrderModel createOrderModel(final CurrencyModel currencyModel, final boolean isActive)
		{
			...
			selectedAccommodationModel.add(testSetup.createSelectedAccommodationModel(entry2.getTravelOrderEntryInfo()));
			final AbstractOrderEntryModel entry3 = createAbstractOrderEntryModel(
					createOrderEntryModel(Collections.singletonList(createTaxValue("YQ", 10d, true, 10d, "EUR"))), isActive,
					AmendStatus.NEW, OrderEntryType.TRANSPORT,
					createTravelOrderEntryInfoModel(1, Collections.singletonList(transportOfferingModelInbound), travelRoute), null,
					null, product3, 1, 100d, 100d, 3, order, "TEST_ITEM_ID_TYPE");
			selectedAccommodationModel.add(testSetup.createSelectedAccommodationModel(entry3.getTravelOrderEntryInfo()));
