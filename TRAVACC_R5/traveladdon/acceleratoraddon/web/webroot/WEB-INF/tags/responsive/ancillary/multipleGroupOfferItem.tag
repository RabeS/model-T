	...
	<div class="col-xs-4 col-sm-5 form-group">
		<select class="form-control y_selectAncillaryProduct" data-offertabid="${offerGroupIndex}_${tabIndex}" data-offertype='multiple' data-itemindex="${itemindex}">
			...
			<c:forEach var="offerPricingInfo" items="${offerPricingInfos}">
				<c:set var="travellerMinOfferQty" value="${offerPricingInfo.travelRestriction.travellerMinOfferQty > 1 ? offerPricingInfo.travelRestriction.travellerMinOfferQty : 1}" />
				...
				<c:forEach var="traveller" items="${travellers}">
					<c:if test="${fn:contains(offerPricingInfo.travelRestriction.passengerTypes, traveller.travellerInfo.passengerType.code)}">
						<c:set var="displayProduct" value="true"/>
					</c:if>
				</c:forEach>
				<c:choose>
				...
				<c:if test="${empty fn:trim(passengerTypes) || displayProduct}">
