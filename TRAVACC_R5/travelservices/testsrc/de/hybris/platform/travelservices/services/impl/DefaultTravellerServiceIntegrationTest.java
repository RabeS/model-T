...
/**
 * Integration Test for the TravellerService implementation using ServicelayerTransactionalTest
 */
@IntegrationTest
public class DefaultTravellerServiceIntegrationTest extends ServicelayerTransactionalTest
{
	...
	public void setUp()
	{
		...
		customer.setUid("bad.guy1@gmail.com");
		customer.setName("Adult1");

		traveller = new TravellerModel();
		traveller.setLabel("adult1");
		traveller.setType(TravellerType.PASSENGER);
		traveller.setUid("001122334655");

		final PassengerTypeModel passengerType = new PassengerTypeModel();
		passengerType.setCode("adult1");
		passengerType.setName("Adult1", Locale.ENGLISH);
	...
	{
		final List<TravellerModel> travellers = travellerService.findSavedTravellersUsingFirstNameText("asd", "adult1", customer);
	...
	{
		final List<TravellerModel> travellers = travellerService.findSavedTravellersUsingLastNameText("asd", "adult1", customer);
		Assert.assertTrue(CollectionUtils.isNotEmpty(travellers));
	}
}
