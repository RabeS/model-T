ACC.accommodationdetails = {
    ...
    bindAmenitiesList: function () {
        ...
        $('.show-all', amenities).on("click", function (e) {
            e.preventDefault();
            $('.amenities-items li > ul').slideDown('fast', function(){
                continueBarAffix();
            });
        ...
        $('.collapse-all', amenities).on("click", function (e) {
            e.preventDefault();
            $('.amenities-items li > ul').slideUp('fast', function(){
                continueBarAffix();
            });
            $(this).addClass('hidden');
            $('.show-all').removeClass('hidden');
    ...
    bindAddToCart: function () {
        ...
        var $numberOfRooms = $('.y_numberOfRooms'),
        $continueButton = $(".y_accommodationDetailsContinue");
        ...
        $numberOfRooms.on("focus click", function(){
            previousSelectedValue=$(this).val();
        }).on("change", function () {
            $('.y_spinner').removeClass('hidden');
            $continueButton.prop("disabled", "disabled");
                ...
                $.when(ACC.services.addAccommodationToCartAjax($("#accommodationAddToCartForm"))).then(
                    function (response) {
                        ...
                        if (jsonData.valid) {
                            ...
                            $numberOfRooms.each(function(index){
                                if($(this).val() > 0) {
                                    $continueButton.removeAttr("disabled");
                                    return false;
                                } else {
                            ...
                            $("#y_addAccommodationToCartErrorModal").dialog('open');
    		...
    		$("select[id^='"+similarRoomStayId+"']").each(function (index, element) {
    			if(currentNoOfRoomsSelectionId!=$(element).attr('id')) {
    				...
    				if(totalRoomSelectedForSameRoomType>maxStockQuantity){
    					...
    					$(".y_ratePlanRoomSelectStockErrorModal").dialog('open');
            ...
            $.when(ACC.services.validateAccommodationCartAjax()).then(
                function (response) {
                    if (response.valid) {
                        ...
                        $("#y_validateCartErrorModal").dialog('open');
                    }
                });
        });

        $(document).bind('click', '.y_ratePlanRoomSelectStockErrorModal .close, .ui-widget-overlay', function () {
    ...
    showNoAvailabilityModal : function() {
        if($("#y_noAccommodationAvailability"))
        {
            ...
            if(noAccommodationAvailability == "show"){
                $("#y_noAvailabilityModal").dialog('open');
            }
        }
    },
