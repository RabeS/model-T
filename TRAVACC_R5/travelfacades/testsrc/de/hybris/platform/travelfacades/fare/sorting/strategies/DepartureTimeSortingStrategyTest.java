...
/**
 * Unit Test for {@link DepartureTimeSortingStrategy}
 */
@UnitTest
@RunWith(MockitoJUnitRunner.class)
public class DepartureTimeSortingStrategyTest
{
	@InjectMocks
	DepartureTimeSortingStrategy departureTimeSortingStrategy;
	...
	private void setUpPricedItineraries(final List<PricedItineraryData> pricedItineraries)
	{
		...
		final TransportOfferingData toData1 = createTransportOfferingData(0);
		final TransportOfferingData toData2 = createTransportOfferingData(1);
		final TransportOfferingData toData3 = createTransportOfferingData(2);
	...
	public void arrangeItinerariesInDecreasingOrder(final List<PricedItineraryData> pricedItineraries)
	{

		for (int i = pricedItineraries.size() - 1; i >= 0; i--)
		{
			pricedItineraries.get((pricedItineraries.size() - 1) - i).getItinerary().getOriginDestinationOptions().get(0)
					.getTransportOfferings().get(0).setDepartureTime(TravelDateUtils.addHours(new Date(), i));
		}
	}
	...
	@Test
	public void testSortDataWhenSetInDecreasingOrder()
	{
		final FareSelectionData fareSelectionData = new FareSelectionData();
		arrangeItinerariesInDecreasingOrder(pricedItineraries);
		fareSelectionData.setPricedItineraries(pricedItineraries);
		departureTimeSortingStrategy.sortFareSelectionData(fareSelectionData);
		final Date expectedDate = expectedPricedItineraries.get(0).getItinerary().getOriginDestinationOptions().get(0)
				.getTransportOfferings().get(0).getDepartureTime();
		final Date actualDate = pricedItineraries.get(0).getItinerary().getOriginDestinationOptions().get(0).getTransportOfferings()
				.get(0).getDepartureTime();
		Assert.assertEquals(expectedDate.toString(), actualDate.toString());
	}

	@Test
	public void testSortDataWhenSetInIncreasingOrder()
	{
		final FareSelectionData fareSelectionData = new FareSelectionData();
		fareSelectionData.setPricedItineraries(pricedItineraries);
		departureTimeSortingStrategy.sortFareSelectionData(fareSelectionData);
