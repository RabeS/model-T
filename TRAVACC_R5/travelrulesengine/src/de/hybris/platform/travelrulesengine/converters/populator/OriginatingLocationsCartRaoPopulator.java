...
public class OriginatingLocationsCartRaoPopulator implements Populator<AbstractOrderModel, CartRAO>
{
			...
			throws ConversionException
	{
		...
		else if (BookingJourneyType.BOOKING_ACCOMMODATION_ONLY.equals(source.getBookingJourneyType()))
		{
			// MOCK OriginatingLocations
			// We assume that the location from which the request is originated is corresponding to the accommodation Location.
			final Optional<AbstractOrderEntryModel> accommodationEntry = source.getEntries().stream()
					.filter(entry -> entry.getEntryGroup() != null).findAny();

			if (!accommodationEntry.isPresent())
			{
				return;
			}
			final Set<String> accommodationLocations = new HashSet<>();
			populateLocationsFromLocation(
					((AccommodationOrderEntryGroupModel) accommodationEntry.get().getEntryGroup()).getAccommodationOffering()
							.getLocation(), accommodationLocations);

			target.setOriginatingLocations(accommodationLocations);
		}
	}
