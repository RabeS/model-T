    ...
    <c:choose>
        <c:when test="${actionType == 'ADD_REQUEST' || actionType == 'CANCEL_REQUEST' || actionType == 'CHANGE_DATES' || actionType=='WRITE_REVIEW' || actionType == 'AMEND_EXTRAS'}">
            ...
            <c:if test="${actionType == 'AMEND_EXTRAS'}">
                ...
                <a class="btn btn-primary btn-block ${fn:escapeXml(y_class)} y_bookingAction disabled" href="${amendExtrasURL}" ${disabled ? 'disabled' : ''}>
        ...
        <c:otherwise>
            ...
            <a class="btn btn-primary btn-block y_bookingAction ${fn:escapeXml(y_class)} disabled" href="${isDynamicPackage?'':actionUrl}"  data-bookingcode="${fn:escapeXml(bookingReferenceCode)}">
                <spring:theme code="button.booking.details.accommodation.booking.action.${actionType}" text="${fn:escapeXml(fn:toLowerCase(actionType))}" />
            </a>
        </c:otherwise>
    </c:choose>
