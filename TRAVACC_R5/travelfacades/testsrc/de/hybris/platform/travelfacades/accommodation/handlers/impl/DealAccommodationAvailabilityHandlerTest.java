...
/**
 * Unit test class for {@link DealAccommodationAvailabilityHandler}
 */

@UnitTest
@RunWith(MockitoJUnitRunner.class)
public class DealAccommodationAvailabilityHandlerTest
{
	@InjectMocks
	private DealAccommodationAvailabilityHandler handler;
	...
	public void testPopulate() throws ParseException
	{
		...
		given(commerceStockService.getStockLevelQuantity(productModel, Collections.singletonList(accommodationOfferingModel)))
				.willReturn(10L);
		given(ratePlanModel.getCode()).willReturn("rateplan1");
		final BundleTemplateModel bundleProduct = testDataSetUp.createBundleTemplateModel(Collections.singletonList(productModel));
		given(bundleTemplateService.getBundleTemplateForCode(Matchers.anyString())).willReturn(bundleProduct);
	...
	private class TestDataSetUp
	{
		...
		private BundleTemplateModel createBundleTemplateModel(final List<ProductModel> products)
		{
			final BundleTemplateModel bundleTemplateModel = new BundleTemplateModel();
			bundleTemplateModel.setProducts(products);
			return bundleTemplateModel;

		}

		private RoomRateProductModel createRoomRateProduct(final List<DateRangeModel> dateRanges)
		{
