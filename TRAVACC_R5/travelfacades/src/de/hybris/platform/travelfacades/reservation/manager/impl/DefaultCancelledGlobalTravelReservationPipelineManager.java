 ...
 */
public class DefaultCancelledGlobalTravelReservationPipelineManager implements GlobalTravelReservationPipelineManager
{
			...
			final GlobalTravelReservationData globalTravelReservationData)
	{
		if (getBookingService().checkIfAnyOrderEntryByType(abstractOrderModel, OrderEntryType.TRANSPORT)
				&& !OrderStatus.CANCELLED.equals(abstractOrderModel.getTransportationOrderStatus()))
		{
			globalTravelReservationData.setReservationData(getReservationPipelineManager().executePipeline(abstractOrderModel));
		}
		if (getBookingService().checkIfAnyOrderEntryByType(abstractOrderModel, OrderEntryType.ACCOMMODATION)
				&& !OrderStatus.CANCELLED.equals(abstractOrderModel.getAccommodationOrderStatus()))
		{
			globalTravelReservationData.setAccommodationReservationData(
					getAccommodationReservationPipelineManager().executePipeline(abstractOrderModel));
		}
