...
public class NDCTravellerStatusStrategyTest
{
	@InjectMocks
	NDCTravellerStatusStrategy strategy;
	...
	{
		strategy.setNotAllowedStatusList(
				Stream.of(ConsignmentStatus.CHECKED_IN, ConsignmentStatus.CANCELLED).collect(Collectors.toList()));
	...
	@Test
	public void testFilterOffer() throws NDCOrderException
	{
		...
		Assert.assertTrue(strategy.filterOffer(order, transportOfferings, Collections.singletonList("TEST_UID")));
	}
	...
	private class TestSetup
	{
		...
		public OrderModel createOrderModel(final CurrencyModel currencyModel, final boolean isActive)
		{
			...
			final ConsignmentModel consignment1 = testSetup.createConsignmentModel(entry1);
			...
			final AbstractOrderEntryModel entry2 = createAbstractOrderEntryModel(
					...
					"TEST_ITEM_ID_TYPE");
			final ConsignmentModel consignment2 = testSetup.createConsignmentModel(entry2);
			final AbstractOrderEntryModel entry3 = createAbstractOrderEntryModel(
					...
					"TEST_ITEM_ID_TYPE");
			final ConsignmentModel consignment3 = testSetup.createConsignmentModel(entry3);
			order.setEntries(Stream.of(entry1, entry2, entry3).collect(Collectors.toList()));
			order.setConsignments(Stream.of(consignment1, consignment2, consignment3).collect(Collectors.toSet()));
		...
		private ConsignmentModel createConsignmentModel(final AbstractOrderEntryModel entry)
		{
			...
			consignmentEntry.setOrderEntry(entry);
