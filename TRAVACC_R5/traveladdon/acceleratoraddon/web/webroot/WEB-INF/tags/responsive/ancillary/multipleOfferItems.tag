		...
		<div class="row input-row">
				...
				<c:forEach var="travellerBreakdownDataOuterLoop" items="${offerPricingInfoDataOuterLoop.travellerBreakdowns}">
					<c:if test="${travellerBreakdownDataOuterLoop.traveller.label == traveller.label}">
						<c:if test="${travellerBreakdownDataOuterLoop.quantity >= 1}">
							...
							<div id="${fn:escapeXml(fn:toLowerCase(offerGroupCode))}${fn:escapeXml(index+1)}-tab${fn:escapeXml(tabIndex+1)}ancillary${fn:escapeXml(outerLoop.index+1)}" class="${fn:escapeXml(fn:toLowerCase(offerGroupCode))}${fn:escapeXml(index+1)}-tab${fn:escapeXml(tabIndex+1)}ancillary y_ancillaryOfferItem form-inline full-width clearfix">
			...
			<c:if test="${selectedOffer == 'false' || selectedOfferNum < travellerMinOfferGroupQty}">
				<c:forEach begin="${selectedOfferNum + 1}" end="${travellerMinOfferGroupQty != 0 ? travellerMinOfferGroupQty : 1}" var="idx">
					<div id="${fn:escapeXml(fn:toLowerCase(offerGroupCode))}${fn:escapeXml(index+1)}-tab${fn:escapeXml(tabIndex+1)}ancillary${fn:escapeXml(idx)}" class="${fn:escapeXml(fn:toLowerCase(offerGroupCode))}${fn:escapeXml(index+1)}-tab${fn:escapeXml(tabIndex+1)}ancillary y_ancillaryOfferItem form-inline full-width clearfix">
						<div class="col-xs-6 col-sm-6 form-group">
