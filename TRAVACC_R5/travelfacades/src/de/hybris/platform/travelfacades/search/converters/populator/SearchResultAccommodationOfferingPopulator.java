		...
		implements Populator<SearchResultValueData, AccommodationOfferingDayRateData>
{
	...
	public void populate(final SearchResultValueData source, final AccommodationOfferingDayRateData target)
	{
		...
		final ArrayList<String> brands = getValue(source, TravelfacadesConstants.SOLR_FIELD_BRAND);
		if (CollectionUtils.isNotEmpty(brands))
		{
			final AccommodationProviderModel accommodationProviderModel = getAccommodationProviderService()
					.getAccommodationProviderForProviderCode(brands.get(0));
			if (accommodationProviderModel != null)
			{
				target.setProviderName(accommodationProviderModel.getName());
			}
		}

		target.setBoosted(this.<Boolean> getValue(source, TravelfacadesConstants.SOLR_FIELD_BOOSTED));
		target.setIgnoreRules(this.<Boolean> getValue(source, TravelfacadesConstants.SOLR_FIELD_IGNORE_RULES));
		target.setAccommodationOfferingTypes(getValue(source, TravelfacadesConstants.SOLR_FIELD_PROPERTY_TYPE));
	}
