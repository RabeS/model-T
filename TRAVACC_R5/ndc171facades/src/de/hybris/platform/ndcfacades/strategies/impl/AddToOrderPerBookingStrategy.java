...
/**
 * Strategy to add to the specified order a PER_BOOKING ancillary.
 */
public class AddToOrderPerBookingStrategy extends AbstractAddAncillariesToOrderRestrictionStrategy
{
	@Override
			...
			final int originDestinationRefNumber, final List<AbstractOrderEntryModel> orderEntries) throws NDCOrderException
	{
		getNdcProductFacade()
				.checkIfProductCanBeAddedToTraveller(order, "", ancillaryProduct, NdcfacadesConstants.DEFAULT_ANCILLARY_QUANTITY,
						Collections.emptyList(), routeCode);

		add(order, ancillaryProduct, null, Collections.emptyList(), offerItemID, null, originDestinationRefNumber, orderEntries);
	}
}
