...
public class CreateInventoryForBookingClassStepHandler implements FlowActionHandler
{
	...
	private final static String FIRST_ANCILLARY_PRODUCT_REF_EDITOR_ID = "AncillaryProduct1";
	...
	public void perform(final CustomType customType, final FlowActionHandlerAdapter adapter, final Map<String, String> parameters)
	{
		if (Objects.nonNull(adapter.getWidgetInstanceManager().getWidgetslot()))
		{
			...
			else
			{
				...
				adapter.getWidgetInstanceManager().getModel().remove(TravelbackofficeConstants.FROM_PREVIOUS_STEP);
				adapter.getWidgetInstanceManager().getModel().remove(FIRST_ANCILLARY_PRODUCT_REF_EDITOR_ID);
				adapter.next();
			}
		}
	}
