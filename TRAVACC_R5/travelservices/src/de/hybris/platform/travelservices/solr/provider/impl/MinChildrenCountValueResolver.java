...
/**
 * Resolves the value for minimum number of children (children + infants) for given MarketingRatePlanInfo.
 * If extraGuest field is not set on the model, default value is set to 0.
 */
public class MinChildrenCountValueResolver extends AbstractValueResolver<MarketingRatePlanInfoModel, Object, Object>
{
	private static final int DEFAULT_MIN_CHILDREN_COUNT = 0;
			...
			final ValueResolverContext<Object, Object> resolverContext) throws FieldValueProviderException
	{
		...
		if (CollectionUtils.isNotEmpty(extraGuests))
		{
			...
			{
				if (StringUtils
						.equalsIgnoreCase(extraGuest.getPassengerType().getCode(), TravelservicesConstants.PASSENGER_TYPE_CODE_CHILD)
						|| StringUtils
						.equalsIgnoreCase(extraGuest.getPassengerType().getCode(), TravelservicesConstants.PASSENGER_TYPE_CODE_INFANT))
				{
					childrenCount += extraGuest.getQuantityMin();
				}
			}
		...
		{
			document.addField(indexedProperty, DEFAULT_MIN_CHILDREN_COUNT, resolverContext.getFieldQualifier());
		}
	}
}
