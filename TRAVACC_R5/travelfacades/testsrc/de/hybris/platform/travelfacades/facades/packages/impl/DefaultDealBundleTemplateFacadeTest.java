...
public class DefaultDealBundleTemplateFacadeTest
{
	...
	{
		final String departureDate = TravelDateUtils.convertDateToStringDate(new Date(), TravelservicesConstants.DATE_PATTERN);
		final LocalDate localDate = timeService.getCurrentDateWithTimeNormalized().toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
		final int daysInMonth = YearMonth.of(localDate.getYear(), localDate.getMonth()).lengthOfMonth();
		Assert.assertTrue(CollectionUtils.size(defaultDealBundleTemplateFacade.getDealValidDates("0 0 0 ? * *", departureDate))
				>= daysInMonth - 1);
	}
