...
<c:if test="${not displayAncillaryData}">
	<c:forEach items="${reservationData.reservationItems}" var="reservationItem">
		<c:forEach items="${reservationItem.reservationPricingInfo.selectedSeats}" var="selectedSeats">
			<c:if test="${selectedSeats.traveller.label eq travellerData.label}">
				<c:set var="displayAncillaryData" value="true" />
			</c:if>
		</c:forEach>
	</c:forEach>
...
<div class="modal fade y_additionalInformationModal" id="ancillariesModal_${currentIndex}" tabindex="-1" role="dialog" aria-labelledby="ancillariesModal_${currentIndex}">
