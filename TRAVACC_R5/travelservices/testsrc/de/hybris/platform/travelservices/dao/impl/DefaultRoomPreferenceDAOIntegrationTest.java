...
/**
 * The type Default room preference dao integration test.
 */
@IntegrationTest
public class DefaultRoomPreferenceDAOIntegrationTest extends ServicelayerTransactionalTest
{
	@Resource
	RoomPreferenceDAO roomPreferenceDAO;
	...
	String roomPreferenceType = "BED_PREFERENCE";

	@Test
	public void testGetRoomPreferencesWithType()
	{
		final List<RoomPreferenceModel> rommPreferences = roomPreferenceDAO.getRoomPreferences(roomPreferenceType);
		Assert.assertTrue(CollectionUtils.isNotEmpty(rommPreferences));
	}

	@Test
	public void testGetRoomPreferencesWithCode()
	{
		final List<RoomPreferenceModel> rommPreferences = roomPreferenceDAO.getRoomPreferences(roomPreferenceType);
		final String roomPreferenceCode = rommPreferences.stream().findFirst().get().getPk().getLongValueAsString();
		final List<RoomPreferenceModel> rommPreferencesWithCode = roomPreferenceDAO
				.getRoomPreferences(Collections.singletonList(roomPreferenceCode));

		Assert.assertTrue(CollectionUtils.isNotEmpty(rommPreferencesWithCode));
	}
