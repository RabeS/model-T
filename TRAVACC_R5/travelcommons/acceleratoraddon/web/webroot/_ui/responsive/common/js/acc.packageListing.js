    ...
    bindPriceRangeSelector : function() {
        ...
        if(priceRangeSelected) {
            ...
            if(priceRangelimitArr.length>1) {
                ...
                resultsViewType=new String($('.y_packageListingPageParams').data('resultviewtype'));
                pageNumber = $(".y_packageListingShowMore").data('pagenumber') ? Number($(".y_packageListingShowMore").data('pagenumber')) : 1;
                ACC.packagelisting.showPackagesWithInPriceRangeFilter(resultsViewType,pageNumber,priceRangeSelected);
            }
        }
