...
public class DealAccommodationPriceHandler extends AbstractAccommodationPriceHandler
{
			...
			final BundleTemplateModel bundleTemplate, final String currencyIso)
	{
		...
		final boolean isNet = isNetPrice(roomRateProduct);
		final Double roomRateBundleBasePrice = getPriceValue(priceInformations);
		final List<TaxData> roomRateBundleTaxes = getTaxes(roomRateProduct.getEurope1Taxes(), currencyIso, roomRateBundleBasePrice,isNet);
		final Double roomRateBundleTaxesValue = getTaxesValue(roomRateBundleTaxes);
		final Double roomRateActualPrice = isNet? Double.sum(roomRateBundleBasePrice, roomRateBundleTaxesValue) : roomRateBundleBasePrice;

		// Price of product
		priceInformations = getPriceService().getPriceInformationsForProduct(roomRateProduct);
		final Double roomRateBasePrice = getPriceValue(priceInformations);
		final List<TaxData> roomRateTaxes = getTaxes(roomRateProduct.getEurope1Taxes(), currencyIso, roomRateBasePrice,isNet);
		final Double roomRateTaxesValue = getTaxesValue(roomRateTaxes);
		final Double roomRateWasPrice = isNet? roomRateBasePrice + roomRateTaxesValue : roomRateBasePrice;
