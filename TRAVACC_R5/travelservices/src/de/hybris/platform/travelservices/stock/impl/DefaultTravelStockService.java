...
public class DefaultTravelStockService extends DefaultStockService implements TravelStockService
{
	...
	@Override
	public StockLevelModel createStockLevel(final WarehouseModel warehouse, final StockLevelAttributesModel stockLevelAttribute)
	{
		if (StringUtils.isBlank(stockLevelAttribute.getCode()))
		{
			LOG.warn("Product code empty. Stock not created for warehouse [" + warehouse.getCode() + "].");
	...
	@Override
	public StockLevelModel createStockLevel(final WarehouseModel warehouse, final StockLevelAttributesModel stockLevelAttribute, final Date date)
	{
		if (StringUtils.isBlank(stockLevelAttribute.getCode()))
		{
			LOG.warn("Product code empty. Stock not created for warehouse [" + warehouse.getCode() + "].");
		...
		{
			LOG.warn("Date cannot be null.");
		}
		StockLevelModel stockLevel = null;
		try
		{
			stockLevel = getTravelStockLevelDao().findStockLevel(stockLevelAttribute.getCode(), Arrays.asList(warehouse), date);
		}
		catch (final ModelNotFoundException ex)
		{
			LOG.debug("No stock found for product [" + stockLevelAttribute.getCode() + "] in warehouse [" + warehouse.getCode()
