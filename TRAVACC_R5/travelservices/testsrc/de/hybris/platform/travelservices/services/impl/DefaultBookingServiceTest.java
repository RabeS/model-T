...
import de.hybris.platform.travelservices.strategies.payment.impl.DefaultGlobalDiscountCalculationStrategy;
import de.hybris.platform.travelservices.strategies.payment.impl.TransportGlobalDiscountCalculationStrategy;
...
public class DefaultBookingServiceTest
{
	...
	@Mock
	private TransportGlobalDiscountCalculationStrategy transportGlobalDiscountCalculationStrategy;
	@Mock
	private DefaultGlobalDiscountCalculationStrategy globalDiscountCalculationStrategy;
	...
	public void getOrderTotalPriceByTypeTestWithNet()
	{
		...
		Mockito.when(order.getSubtotal()).thenReturn(50d);
	...
	public void getOrderTotalPriceByTypeTestWithTaxValues()
	{
		...
		Mockito.when(travelCartService.isNetPricedEntry(anyObject())).thenReturn(Boolean.TRUE);
		bookingService.setEntryTypeToDiscountCalculationStrategyMap(entryTypeToDiscountCalculationStrategyMap);
		Mockito.when(entryTypeToDiscountCalculationStrategyMap.get(OrderEntryType.DEFAULT))
				.thenReturn(globalDiscountCalculationStrategy);
		Mockito.when(entryTypeToDiscountCalculationStrategyMap
				.getOrDefault(orderEntryModel.getType(), entryTypeToDiscountCalculationStrategyMap.get(OrderEntryType.DEFAULT)))
				.thenReturn(transportGlobalDiscountCalculationStrategy);
		final List<AbstractOrderEntryModel> entries = order.getEntries().stream()
				.filter(entry -> OrderEntryType.TRANSPORT.equals(entry.getType())).collect(
						Collectors.toList());
		Mockito.when(entryTypeToDiscountCalculationStrategyMap
				.getOrDefault(orderEntryModel.getType(), entryTypeToDiscountCalculationStrategyMap.get(OrderEntryType.DEFAULT))
				.getGlobalDiscount(order, entries)).thenReturn(BigDecimal.valueOf(0));
		assertEquals(Double.valueOf(80), bookingService.getOrderTotalPriceByType(order, OrderEntryType.TRANSPORT));
	}
