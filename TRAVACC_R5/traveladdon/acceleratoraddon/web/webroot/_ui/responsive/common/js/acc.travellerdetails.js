ACC.travellerdetails = {
	...
	_autoloadTracc: [
		"repositioningAdditionalInformationModals",
	...
	repositioningAdditionalInformationModals: function () {
		$(".y_additionalInformationModal").each(function () {
			var positionId = $(this).data("positionid");
			var position = $("[id=" + positionId + "]");
			$(this).parent().appendTo(position);
		})
	},
	...
	bindClearAdditionalInformation: function () {
		...
		$('.y_additionalInformationModal').on('dialogopen', function (e) {
	...
	populateFormFields: function ($form, travellerInfo, userId) {
		...
		if (travellerInfo.gender) {
			$form.find('input:radio.y_passengerGender[value=' + travellerInfo.gender + ']').prop('checked', true);
		}
		else {
			$form.find('input:radio.y_passengerGender[value=male]').prop('checked', true);
		}
		...
		if (ACC.travellerdetails.isAdditionalInformationPopulated($form) && ACC.travellerdetails.isFrequentFlierValid($form)) {
	...
	hideAdditionalInformationModal: function () {
		$(".y_saveAdditionalChanges, .y_modalCloseButton").on('click', function () {
			$(this).closest('.y_additionalInformationModal').dialog('close');
			//the following line is a temporary fix for the locked page
	...
	checkIfAnyValuePresentInsideModelOnLoad: function () {
		$(ACC.travellerdetails.currentForm).each(function () {
			if (ACC.travellerdetails.isAdditionalInformationPopulated($(this))) {
	...
	checkIfAnyValuePresentInsideModelOnFocusOut: function () {
		$(document).on('dialogbeforeclose', ".y_additionalInformationModal", function (e) {
			var $form = $(this).closest(ACC.travellerdetails.currentForm);
			if (!ACC.travellerdetails.isAdditionalInformationPopulated($form)) {
				$form.find('.y_checkedIcon').addClass('hidden');
				$form.find('.y_errorIcon').addClass('hidden');
				return;
			}

			if (ACC.travellerdetails.isFrequentFlierValid($form)) {
				$form.find('.y_checkedIcon').removeClass('hidden');
				$form.find('.y_errorIcon').addClass('hidden');
			}
			else {
	...
	isAdditionalInformationPopulated: function ($form) {
		...
		return email !== '' || reasonOfTravel !== '' || specialAssistance || frequentFlier;
	},

	isFrequentFlierValid: function ($form) {
		var frequentFlier = $form.find('.y_travellerDetailsMembershipYesBtn').is(':checked');
		var frequentFlierNumber = $form.find('.y_passengerFrequentFlyerMembershipNumber').val();
		return frequentFlier ? frequentFlierNumber !== '' : true;
	},
	...
	bindSaveAllTravellers: function () {
		var savedDetailCheckbox = $('.y_savedetails');
		var saveAllTravellers = $('.y_saveAllTravellers');

		savedDetailCheckbox.prop('checked', false);
		saveAllTravellers.prop('checked', false);
		saveAllTravellers.on('click', function () {
			var closestSavedDetails = $(this).closest('.y_travellerDetailsParentDiv').find('.y_savedetails');
			if ($(this).is(':checked')) {
				closestSavedDetails.prop('checked', true);
			}
			else {
				closestSavedDetails.prop('checked', false);
			}
		});

		savedDetailCheckbox.on('click', function () {
			var closestParent = $(this).closest('.y_travellerDetailsParentDiv');
			var closestSaveAll = closestParent.find(saveAllTravellers);
			var closestSavedDetailCheckboxes = closestParent.find('.y_savedetails');

			if ($(this).is(':not(:checked)')) {
				closestSaveAll.prop('checked', false);
			}
			if (closestSavedDetailCheckboxes.not(':checked').length === 0) {
				closestSaveAll.prop('checked', true);
			}
		});
	},
