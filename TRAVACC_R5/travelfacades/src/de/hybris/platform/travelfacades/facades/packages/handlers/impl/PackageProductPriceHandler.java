...
public class PackageProductPriceHandler implements StandardPackageProductHandler
{
	...
	/**
	 * Returns the RateData properly populated given the productModel and the bundleTemplate. The wasRate is the standard price of
	 * the product taken from the priceRow for the current Currency. The actualRate is the price determined by the
	 * changeProductPriceBundleRule if it exist, otherwise will be the same as the wasRate.
	 *
	 * @param product
	 * 		as the productModel
	...
	protected RateData getPriceForProductAndBundle(final ProductModel product, final BundleTemplateModel bundleTemplate)
	{
		...
		final Boolean isNet = getTravelCommercePriceFacade().isNetPriceProduct(product);
		final Double productBasePrice = priceInformation.getPriceValue().getValue();
		final List<TaxData> productTaxes = getTaxes(product.getEurope1Taxes(), currencyIso, productBasePrice,isNet);
		final Double productTaxesValue = productTaxes.stream().mapToDouble(tax -> tax.getPrice().getValue().doubleValue()).sum();
		final Double productWasPrice = isNet ? Double.sum(productBasePrice, productTaxesValue) : productBasePrice;
		...
		final List<TaxData> productBundleTaxes = getTaxes(product.getEurope1Taxes(), currencyIso, productBundleBasePrice,isNet);
		final Double productBundleTaxesValue = productBundleTaxes.stream()
				.mapToDouble(tax -> tax.getPrice().getValue().doubleValue()).sum();
		final Double productActualPrice = isNet ?
				Double.sum(productBundleBasePrice, productBundleTaxesValue) :
				productBundleBasePrice;
			...
			final Boolean isNet)
	{
		...
		result.addAll(taxRowModels.stream().map(tax -> {
			final TaxData taxData = new TaxData();
			final Double taxValue = isNet ?
					calculateNetTaxValueForProduct(tax, productBasePrice) :
					calculateGrossTaxValueForProduct(tax, productBasePrice);
			taxData.setPrice(
					getTravelCommercePriceFacade().createPriceData(PriceDataType.BUY, BigDecimal.valueOf(taxValue), currencyIso));
			return taxData;
		}).collect(Collectors.toList()));
	...
	protected Double calculateNetTaxValueForProduct(final TaxRowModel tax, final Double productBasePrice)
	{
		final Double value = tax.getValue();
		return Objects.isNull(tax.getCurrency()) ? productBasePrice / 100 * value : value;
	}

	protected Double calculateGrossTaxValueForProduct(final TaxRowModel tax, final Double productBasePrice)
	{
		final Double value = tax.getValue();
		return Objects.isNull(tax.getCurrency()) ? productBasePrice * value / (100 + value) : value;
	}
