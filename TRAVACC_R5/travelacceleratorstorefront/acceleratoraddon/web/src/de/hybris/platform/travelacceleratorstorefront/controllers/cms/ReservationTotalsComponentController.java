...
public class ReservationTotalsComponentController extends SubstitutingCMSAddOnComponentController<ReservationTotalsComponentModel>
{
	...
	private static final String SESSION_FARE_SELECTION = "sessionFareSelection";
	...
	protected ImageData getImage(final HttpServletRequest request)
	{
		...
		return Objects.isNull(image) ? getFallbackImage() : image;
	}

	protected ImageData getFallbackImage()
	{
		final FareSelectionData fareSelectionData = sessionService.getAttribute(SESSION_FARE_SELECTION);
		return Objects.nonNull(fareSelectionData) ? travelImageFacade.getImageForArrivalTransportFacility(
				fareSelectionData.getPricedItineraries().stream().filter(itinerary -> itinerary.getOriginDestinationRefNumber() == 0)
						.findAny().get().getItinerary().getRoute().getDestination()
						.getCode()) : null;
	}
}
