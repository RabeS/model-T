ACC.farefinder = {
	...
	bindChangeTripTypeButton: function () {
		$(document).find('#y_fareFinderForm .y_fareFinderTripTypeBtn').change(function () {
			if ($('.y_fareFinderTripTypeBtn:checked').val() == 'SINGLE') {
	...
	bindOriginLocationAutosuggest: function () {
		$(".y_originLocation").autosuggestion({
			...
			suggestionFieldChangedCallback: function () {
				...
				if(!$('.y_destinationLocation').val().length === 0) {
					$(".y_destinationLocation").val("");
					$(".y_destinationLocationCode").val("");
				}
			},
			invalidLocationHandler: function () {
				var output = [];
				var errorMessage = ACC.addons.travelacceleratorstorefront["error.autosuggestion.invalid.location"];
				output.push('<div class = "\autocomplete-suggestions no-matching-airports\"><ul><li>' + errorMessage + '</li></ul></div>');

				var suggestionSelect = "#y_originLocationSuggestions";
				$(suggestionSelect).html(output);
				$(suggestionSelect).removeClass("hidden");
			},
