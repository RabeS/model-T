...
public class NDCFlightPriceCurrencyValidator implements NDCRequestValidator<FlightPriceRQ>
{
	...
	protected boolean validateMaxCurrencies(final FlightPriceRQ flightPriceRQ, final ErrorsType errorsType)
	{
		...
		{
			if (flightPriceRQ.getParameters().getCurrCodes().getFiledInCurrency()
					.size() > NdcwebservicesConstants.MAX_CURRENCY_VALUES)
			{
				addError(errorsType,
	 ...
	 * @return boolean
	 */
	protected void validateCurrencyValue(final FlightPriceRQ flightPriceRQ, final ErrorsType errorsType)
	{
		if (Objects.nonNull(flightPriceRQ.getParameters()) && Objects.nonNull(flightPriceRQ.getParameters().getCurrCodes())
				&& CollectionUtils.isNotEmpty(flightPriceRQ.getParameters().getCurrCodes().getFiledInCurrency()))
		{
			final FiledInCurrency filledInCurrency = flightPriceRQ.getParameters().getCurrCodes().getFiledInCurrency().stream()
					.findFirst().get();

			if (Objects.isNull(filledInCurrency.getCurrCode()))
			{
				addError(errorsType,
			...
			if (!isValidIsoCode(filledInCurrency.getCurrCode().getValue()))
			{
				addError(errorsType,
						getConfigurationService().getConfiguration().getString(NdcwebservicesConstants.INVALID_CURRENCY));
				return;
			}
			return;
		}
