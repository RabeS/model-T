...
public abstract class AbstractNDCFacade
{
	...
	protected BundleTemplateModel getFareProductChildBundle(final BundleTemplateModel bundleTemplate)
	{
		for (final BundleTemplateModel childTemplate : bundleTemplate.getChildTemplates())
		{
			...
			if (CollectionUtils.isNotEmpty(products) && (products.get(0) instanceof FareProductModel
					|| ProductType.FARE_PRODUCT.equals(products.get(0).getProductType())))
			{
				return childTemplate;
			}
		}
	 ...
	 */
	protected PassengerTypeModel getPassengerTypeModel(final PTC ptc)
	{
		final PassengerTypeModel passengerType = getNdcPassengerTypeService().getPassengerType(ptc.getValue());
