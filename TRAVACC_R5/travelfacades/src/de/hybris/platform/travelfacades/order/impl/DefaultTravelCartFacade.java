...
public class DefaultTravelCartFacade extends DefaultSubscriptionCartFacade implements TravelCartFacade
{
	...
	private List<SelectedAccommodationValidationStrategy> selectedAccommodationValidationStrategyList;
	...
	public CartModificationData addToCart(final String code, final long quantity) throws CommerceCartModificationException
	{
		...
		parameter.setOrderEntryType(OrderEntryType.TRANSPORT);
	...
	/**
	 * Method to add additional properties like Transport offering Code to a cart entry of a particular bundle using the default
	 * addToCartCriteria
	 *
	 * @param productCode
	 ...
	 * @deprecated Deprecated since version 2.0. Replaced by {@link #addPropertiesToCartEntry(String, int, List, String, int,
	 * String, Boolean, AmendStatus, String)}
	 */
	@Deprecated
	 ...
	 * {@inheritDoc}
	 *
	 * @deprecated Deprecated since version 2.0. Use {@link #addPropertiesToCartEntry(String, int, List, String, int, String,
	 * Boolean, AmendStatus, String)} instead.
	 */
	@Override
	@Deprecated
	 ...
	 * 		the amend status
	 * @return map map
	 * @deprecated Deprecated since version 2.0. Replaced by {@link #populatePropertiesMap(String, String, int, List, List,
	 * Boolean, AmendStatus)}
	 */
	@Deprecated
			...
			final Boolean active, final AmendStatus amendStatus) throws CommerceCartModificationException
	{
		...
		if (bundleNo < 0)
		{
			cartModifications.add(getTravelBundleCommerceCartFacade()
					.startBundle(bundleTemplateId, productId, quantity, OrderEntryType.TRANSPORT));
		...
		{
			cartModifications.add(getTravelBundleCommerceCartFacade().addToCart(productId, quantity, bundleNo));
			if (CollectionUtils.isNotEmpty(cartModifications))
			{
				if (Objects.nonNull(addBundleToCartParamsMap))
				{
	...
	/**
	 * Returns the code that is assigned to the version ID field in the {@link TravellerModel}. In the current session cart does
	 * not have any original order it is returning null.
	 *
	 * @param currentCartCode
	 * 		the current cart code
	 * @return cart code
	 */
	...
	/**
	 * Method to add product to cart. This will include setting up the params required for pricing and to set travel specific
	 * params in the order entry.
	 *
	 * @param code
	 * 		the code
	 * @param qty
			...
			throws CommerceCartModificationException
	{
		...
		setAddToCartParametersInContext(transportOfferingCodes, travelRouteCode, travellerData, priceLevel,
				originDestinationRefNumber, active, amendStatus);
		final List<CartModificationData> cartModificationDataList = getBundleCartFacade()
				.addToCart(code, qty, bundleNo, bundleTemplateId,
	 ...
	 *
	 * @return the traveller version id from cart
	 */
	...
	public PriceData getTotalToPayPrice()
	{
		...
		if (cartModel.getOriginalOrder() != null)
		{
			...
			final BigDecimal oldTotalWithTaxes = originalOrder.getNet() ? BigDecimal.valueOf(originalOrder.getTotalPrice())
					.add(BigDecimal.valueOf(originalOrder.getTotalTax())) : BigDecimal.valueOf(originalOrder.getTotalPrice());
			final BigDecimal orderTotalPaid = getBookingService().getOrderTotalPaid(originalOrder);
			final BigDecimal newTotalWithTaxes = cartModel.getNet() ? BigDecimal.valueOf(cartModel.getTotalPrice())
					.add(BigDecimal.valueOf(cartModel.getTotalTax())) : BigDecimal.valueOf(cartModel.getTotalPrice());
	...
	/**
	 * @return total to pay after change dates
	...
	public PriceData getTotalToPayPriceAfterChangeDates()
	{
		...
		for (final AbstractOrderEntryModel entry : activeEntries)
		{
			...
			if (CollectionUtils.isNotEmpty(entry.getTaxValues()) && isNetPrice(entry))
			{
		...
		totalCartAmount -= getGlobalDiscountForEntryType(cartModel, OrderEntryType.ACCOMMODATION).doubleValue();
	...
	{
		if (StringUtils.equals(getSessionService().getAttribute(TravelfacadesConstants.SESSION_PAY_NOW), originalOrderCode)
				&& !hasCartBeenAmended())
		{
	...
	{
		final CartModel cartModel = getCartService().getSessionCart();
		final BigDecimal cartTotalWithTaxes = cartModel.getNet() ? BigDecimal.valueOf(cartModel.getTotalPrice())
				.add(BigDecimal.valueOf(cartModel.getTotalTax())) : BigDecimal.valueOf(cartModel.getTotalPrice());
	...
	public boolean validateOriginDestinationRefNumbersInCart()
	{
		...
		final List<AbstractOrderEntryModel> abstractOrderEntries = getTravelCartService().getSessionCart().getEntries().stream()
				.filter(entry -> OrderEntryType.TRANSPORT.equals(entry.getType())
						&& !(entry.getProduct() instanceof FeeProductModel || ProductType.FEE
						.equals(entry.getProduct().getProductType())))
				.collect(Collectors.toList());
	...
	@Override
	public Boolean isNetPrice(final AbstractOrderEntryModel entry)
	{
		return getTravelCartService().isNetPricedEntry(entry);
	}

	@Override
	public BigDecimal getGlobalDiscountForEntryType(final AbstractOrderModel abstractOrder, final OrderEntryType entryType)
	{
		return getTravelCartService().getGlobalDiscountForEntryType(abstractOrder, entryType);
	}

	@Override
	public RateData getTotalsForReservation(final String originalOrderCode)
	{
		return populateRateDataFromCart(originalOrderCode);
	}
	...
	public boolean validateSeatSelection()
	{
		...
		for (final SelectedAccommodationValidationStrategy strategy : getSelectedAccommodationValidationStrategyList())
		{
			if (!strategy.validateSelectedAccommodation(cartModel))
			{
				return false;
			}
		}
	 ...
	 */
	protected RateData populateRateDataFromCart(final String originalOrderCode)
	{
		...
		final CartModel sessionCart = getTravelCartService().getSessionCart();
		final List<AbstractOrderEntryModel> activeEntries = sessionCart.getEntries().stream()
				.filter(entry -> entry.getActive()).collect(
						Collectors.toList());

		final Double totalBasePrice = activeEntries.stream()
				.mapToDouble(entry -> getTravelCartService().isNetPricedEntry(entry) ?
						entry.getTotalPrice() :
						entry.getBasePrice() * entry.getQuantity()).sum();
		rateData.setBasePrice(getTravelCommercePriceFacade().createPriceData(totalBasePrice, 2));

		final Double discounts = getDiscountsToDisplay(sessionCart);
		...
		taxData.setPrice(getTravelCommercePriceFacade().createPriceData(sessionCart.getTotalTax(), 2));
		rateData.setTotalTax(taxData);
		if (StringUtils.equals(getSessionService().getAttribute(TravelfacadesConstants.SESSION_PAY_NOW), originalOrderCode)
				&& !hasCartBeenAmended())
		{
			rateData.setActualRate(getTravelCommercePriceFacade().createPriceData(
					getBookingFacade().getOrderTotalToPayForOrderEntryType(originalOrderCode, OrderEntryType.ACCOMMODATION)
							.doubleValue(),
					2));
		}
		else
		{
			rateData.setActualRate(getTotalToPayPrice());
		}

		rateData.setNet(sessionCart.getNet());

		final Double wasRate = totalBasePrice - discounts + (sessionCart.getNet() ? taxData.getPrice().getValue().doubleValue() : 0);
		rateData.setWasRate(getTravelCommercePriceFacade().createPriceData(wasRate,sessionCart.getCurrency().getIsocode()));
	 ...
	 * @param abstractOrder
	 * 		the abstract order
	 * @return discounts to display
	 */
	protected Double getDiscountsToDisplay(final AbstractOrderModel abstractOrder)
	{
	 ...
	 *
	 * @return the selected accommodation validation strategy list
	 */
	protected List<SelectedAccommodationValidationStrategy> getSelectedAccommodationValidationStrategyList()
	{
		return selectedAccommodationValidationStrategyList;
	}

	/**
	 * Sets selected accommodation validation strategy list.
	 *
	 * @param selectedAccommodationValidationStrategyList
	 * 		the selected accommodation validation strategy list
	 */
	@Required
	public void setSelectedAccommodationValidationStrategyList(
			List<SelectedAccommodationValidationStrategy> selectedAccommodationValidationStrategyList)
	{
		this.selectedAccommodationValidationStrategyList = selectedAccommodationValidationStrategyList;
	}
}
