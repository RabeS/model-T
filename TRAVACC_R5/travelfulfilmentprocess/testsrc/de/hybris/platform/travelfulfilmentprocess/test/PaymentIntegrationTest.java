...
import de.hybris.platform.travelfulfilmentprocess.test.actions.TestActionTemp;
...
/**
 * The type Payment integration test.
 */
@IntegrationTest
public class PaymentIntegrationTest extends ServicelayerTest
{
	...
	public static void prepare() throws Exception //NOPMD
	{
		...
		xmlReader.loadBeanDefinitions(new ClassPathResource(
				"/travelfulfilmentprocess/test/travelfulfilmentprocess-spring-test.xml"));
		xmlReader.loadBeanDefinitions(new ClassPathResource(
				"/travelfulfilmentprocess/test/travelfulfilmentprocess-spring-test-fraudcheck.xml"));
		xmlReader
				.loadBeanDefinitions(new ClassPathResource("/travelfulfilmentprocess/test/process/order-process-spring.xml"));
		...
		definitonFactory.add("classpath:/travelfulfilmentprocess/test/process/payment-process.xml");
	...
	public void setUpProductCatalogue()
	{
		try
		{
			...
			importCsv("/travelfulfilmentprocess/test/testBasics.csv", "windows-1252");
			importCsv("/travelfulfilmentprocess/test/testCatalog.csv", "windows-1252");
	...
	public static void removeProcessDefinitions()
	{
		...
		final XmlBeanDefinitionReader xmlReader = new XmlBeanDefinitionReader((BeanDefinitionRegistry) beanFactory);
		xmlReader.loadBeanDefinitions(new ClassPathResource(
				"/travelfulfilmentprocess/test/travelfulfilmentprocess-spring-testcleanup.xml"));
	...
	protected OrderModel placeTestOrder(final boolean valid) throws InvalidCartException, CalculationException
	{
		...
		cartService.addNewEntry(cart, productService.getProductForCode(version, "testProduct1"), 1, null);
		cartService.addNewEntry(cart, productService.getProductForCode(version, "testProduct2"), 2, null);
		cartService.addNewEntry(cart, productService.getProductForCode(version, "testProduct3"), 3, null);
