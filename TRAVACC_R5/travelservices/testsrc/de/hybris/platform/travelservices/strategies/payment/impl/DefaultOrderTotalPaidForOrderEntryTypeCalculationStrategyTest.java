...
/**
 * unit test for {@link DefaultOrderTotalPaidForOrderEntryTypeCalculationStrategy}
 */
@UnitTest
@RunWith(MockitoJUnitRunner.class)
public class DefaultOrderTotalPaidForOrderEntryTypeCalculationStrategyTest
{
	@InjectMocks
	DefaultOrderTotalPaidForOrderEntryTypeCalculationStrategy defaultOrderTotalPaidForOrderEntryTypeCalculationStrategy;
	...
	public void testCalculate()
	{
		...
		paymentTransactions.add(createPaymentTransaction(OrderEntryType.ACCOMMODATION, PaymentTransactionType.CAPTURE, 100d));
		paymentTransactions
				.add(createPaymentTransaction(OrderEntryType.ACCOMMODATION, PaymentTransactionType.REFUND_STANDALONE, 50d));
		paymentTransactions.add(createPaymentTransaction(OrderEntryType.TRANSPORT, PaymentTransactionType.CAPTURE, 200d));
		...
		Assert.assertEquals(50, defaultOrderTotalPaidForOrderEntryTypeCalculationStrategy
				.calculate(orderModel, OrderEntryType.ACCOMMODATION).intValue());
	}

	@Test
	public void testCalculateForWithoutCapture()
	{
		when(configurationService.getConfiguration()).thenReturn(configuration);
		when(configuration.getDouble(TravelservicesConstants.PAYMENT_TRANSACTION_AUTH_AMOUNT)).thenReturn(0.001d);
		...
		final List<PaymentTransactionModel> paymentTransactions = new ArrayList<>();
		paymentTransactions
				.add(createPaymentTransaction(OrderEntryType.ACCOMMODATION, PaymentTransactionType.REFUND_STANDALONE, 10d));
		final PaymentTransactionModel paymentModel = createPaymentTransaction(OrderEntryType.TRANSPORT,
				PaymentTransactionType.CAPTURE, 10d);
		paymentModel.setAbstractOrderEntries(null);
		paymentTransactions.add(paymentModel);
		...
		Assert.assertEquals(0, defaultOrderTotalPaidForOrderEntryTypeCalculationStrategy
				.calculate(orderModel, OrderEntryType.ACCOMMODATION).intValue());
	}

	@Test
	public void testCalculateForNonExistingOrderEntryType()
	{
		when(configurationService.getConfiguration()).thenReturn(configuration);
		when(configuration.getDouble(TravelservicesConstants.PAYMENT_TRANSACTION_AUTH_AMOUNT)).thenReturn(0.001d);
		...
		Assert.assertEquals(0,
				defaultOrderTotalPaidForOrderEntryTypeCalculationStrategy.calculate(orderModel, OrderEntryType.DEFAULT).intValue());
	}
