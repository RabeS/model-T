...
public class DefaultFlightPriceFacadeTest
{
	...
	private class TestSetup
	{
		...
		private void createAnonymousPassengerList(FlightPriceRQ flightPriceRQ)
		{
			...
			ptc.setValue("TEST_PTC_VALUE");

			AnonymousTravelerType anonymousTraveler = new AnonymousTravelerType();
			anonymousTraveler.setPTC(ptc);

			anonymousTravelerList.getAnonymousTraveler().add(anonymousTraveler);
		...
		public FlightPriceRQ createFlightPriceRQ()
		{
			...
			flightPriceRQ.setTravelers(createTravelers());
			flightPriceRQ.setQuery(createQuery());
			createAnonymousPassengerList(flightPriceRQ);
		...
		private MessageParamsBaseType.CurrCodes createCurrCodes()
		{
			...
			currencyCodes.getCurrCode().add(createCurrCode());
		...
		private Travelers createTravelers()
		{
			final Travelers travellers = new Travelers();
			travellers.getTraveler().add(createTraveler());
			return travellers;
		}

		private Travelers.Traveler createTraveler()
		{
			final Travelers.Traveler traveller = new Travelers.Traveler();
			traveller.getAnonymousTraveler().add(createAnonymousTravelerType());
			return traveller;
		}

		private AnonymousTravelerType createAnonymousTravelerType()
		{
			final AnonymousTravelerType anonymousTravelerType = new AnonymousTravelerType();
			anonymousTravelerType.setPTC(createPTC("TEST_PTC_VALUE"));
			anonymousTravelerType.setObjectKey("TEST_PTC_VALUE");
			return anonymousTravelerType;
		}

		private PTC createPTC(final String code)
		{
			final PTC ptc = new PTC();
			ptc.setValue(code);
			ptc.setQuantity(BigInteger.valueOf(1));
			Mockito.when(ndcPassengerTypeService.getPassengerType(code)).thenReturn(createPassengerTypeModel());
			return ptc;
		}
		...
		private BundleTemplateModel createBundleTemplateModel(final BundleType bundleType)
		{
			final BundleTemplateModel bundleTemplateModel = new BundleTemplateModel()
			{
				...
				{
					return bundleType.getCode();
				}
			};
			bundleTemplateModel.setType(bundleType);
			bundleTemplateModel.setProducts(Stream.of(createProductModel("outbound", ProductType.FARE_PRODUCT),
					createProductModel("PRIORITYCHECKIN", ProductType.ANCILLARY)).collect(Collectors.toList()));
			bundleTemplateModel.setChildTemplates(Collections.singletonList(bundleTemplateModel));
