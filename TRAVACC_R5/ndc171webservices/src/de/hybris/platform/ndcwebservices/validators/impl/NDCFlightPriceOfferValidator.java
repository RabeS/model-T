		...
		implements NDCRequestValidator<FlightPriceRQ>
{
	...
	{
		if (Objects.isNull(flightPriceRQ.getQuery()))
		{
			addError(errorsType,
					getConfigurationService().getConfiguration().getString(NdcwebservicesConstants.QUERY_UNAVAILABLE));
			return;
		}
	...
	protected Map<String, Integer> getOffersPerPTC(final FlightPriceRQ flightPriceRQ) throws NDCOrderException
	{
		...
		final List<PassengerType> passengers = flightPriceRQ.getDataLists().getPassengerList().getPassenger();
		passengers.forEach(passenger -> offersPerPTC.putIfAbsent(passenger.getPTC(), 0));
