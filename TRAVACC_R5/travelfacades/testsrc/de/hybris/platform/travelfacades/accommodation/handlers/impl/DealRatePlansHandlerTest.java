...
/**
 * Unit test class for {@link DealRatePlansHandler}
 */

@UnitTest
@RunWith(MockitoJUnitRunner.class)
public class DealRatePlansHandlerTest
{
	@InjectMocks
	private DealRatePlansHandler handler;
	...
	public void setUp()
	{
		...
		given(ratePlanService.getRatePlanForCode(Matchers.anyString())).willReturn(new RatePlanModel());
		given(ratePlanConverter.convertAll(Matchers.anyList())).willReturn(Arrays.asList(new RatePlanData()));
		doNothing().when(cancelPenaltiesDescriptionCreationStrategy).updateCancelPenaltiesDescription(Matchers.any(),
				Matchers.any());
	}

	@Test
	public void testPopulateForNullRatePlanCode()
	{
		final TestDataSetUp testDataSetUp = new TestDataSetUp();
		...
		criterionData.setRoomStayCandidates(
				Collections.singletonList(testDataSetUp.createRoomStayCandidateData(0, "TEST_RATE_PLAN_CODE")));
		...
		final RoomStayData roomStay = testDataSetUp.createRoomStays(roomtype, 0);
	...
	private class TestDataSetUp
	{
		...
		private RoomStayCandidateData createRoomStayCandidateData(final int roomStayCandidateRefNumber, final String ratePlanCode)
		{
			...
			roomStayCandidateData.setRatePlanCode(ratePlanCode);
