...
ACC.travelcommon = {
		...
		function setSessionPrompt() {
			...
			if (cookieValue) {
				...
				if (timeout <= 300000 && !$("#y_sessionExpirationPreWarningModal").hasClass("show")) {
					$('#y_sessionExpirationPreWarningModal').dialog('open');
	...
	bindSessionContinue: function () {
		$("#y_sessionContinue").on('click', function () {
			$.when(ACC.services.renewSession()).then(function () {
				$("#y_sessionExpirationPreWarningModal").dialog("close");
	...
	bindNavbarActiveItem: function () {
		var navbarItems = $(".y_navbar li").find("a");
		$(navbarItems).each(function () {
			var navbarItem = $(this).attr("href");
			if (navbarItem == window.location.pathname) {
	...
	isValidLocation: function (location) {
		return /^[A-Za-z\u00C0-\u017F\s]+$/i.test(location);
	},
	...
	bindColorboxRemoveInit: function () {
		$(document).on('dialogopen', '.modal', function () {
			$.colorbox.remove();
		});
		$(document).on('dialogclose', '.modal', function () {
