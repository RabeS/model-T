...
public class DefaultAccommodationDao extends DefaultGenericDao<AccommodationModel> implements AccommodationDao
{
	...
	private static final String FIND_BY_ACCOMMODATION_OFFERING_CODE_AND_CATALOG_VERSION =
			"SELECT DISTINCT {a." + AccommodationModel.PK + "} FROM {" + AccommodationModel._TYPECODE + " AS a JOIN "
					+ StockLevelModel._TYPECODE + " AS sl ON {sl." + StockLevelModel.PRODUCTCODE + "} = {a." + AccommodationModel.CODE
					+ "} JOIN " + AccommodationOfferingModel._TYPECODE + " AS ao ON {sl." + StockLevelModel.WAREHOUSE + "} = {ao."
					+ AccommodationOfferingModel.PK + "} JOIN " + CatalogVersionModel._TYPECODE + " AS cv ON {a."
					+ AccommodationModel.CATALOGVERSION + "} = {cv." + CatalogVersionModel.PK + "} } WHERE {ao."
					+ AccommodationOfferingModel.CODE + "} = ?accommodationOfferingCode AND {cv." + CatalogVersionModel.VERSION
					+ "} = ?catalogVersion";
	...
	@Override
	public List<AccommodationModel> findAccommodationForAccommodationOfferingAndCatalog(final String accommodationOfferingCode,
			final String catalogVersion)
	{
		...
		final Map<String, Object> params = new HashMap<String, Object>();
		params.put(ACCOMMODATION_OFFERING_CODE, accommodationOfferingCode);
		params.put(CATALOG_VERSION, catalogVersion);

		final FlexibleSearchQuery flexibleSearchQuery = new FlexibleSearchQuery(
				FIND_BY_ACCOMMODATION_OFFERING_CODE_AND_CATALOG_VERSION, params);
		final SearchResult<AccommodationModel> searchResult = getFlexibleSearchService().search(flexibleSearchQuery);
