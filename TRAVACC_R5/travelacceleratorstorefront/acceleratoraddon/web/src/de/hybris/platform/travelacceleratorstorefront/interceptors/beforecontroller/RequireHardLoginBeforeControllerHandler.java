...
public class RequireHardLoginBeforeControllerHandler implements BeforeControllerHandler
{
	...
	/**
	 * All the pages that are protected by @RequiredHandLogin annotation are checked for the rightful authorization been
	 * done, if not, forces the customer to the Login page. In this overridden method, additional to the out of the box
	 * checks, Manage my booking check has been added as all ManageMyBooking pages are protected with @RequiredHandLogin
	 * annotation.
	 */
	@Override
	...
	/**
	 * Returns true, if there exists a session attribute "manage_my_booking_authentication" indicating that
	 * manageMyBooking authentication has been done.
	 *
	 * @return
	 */
		...
		if (Boolean.TRUE.equals(getSessionService().getAttribute(WebConstants.ANONYMOUS_CHECKOUT)))
		{
			if (getSessionService().getAttribute(WebConstants.ANONYMOUS_CHECKOUT_GUID) == null)
			{
				getSessionService().setAttribute(WebConstants.ANONYMOUS_CHECKOUT_GUID,
						StringUtils.substringAfter(getCartService().getSessionCart().getUser().getUid(), "|"));
			}
			return true;
		}
		return false;
