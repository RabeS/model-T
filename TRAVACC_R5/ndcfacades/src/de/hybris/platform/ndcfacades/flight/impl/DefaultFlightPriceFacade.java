...
public class DefaultFlightPriceFacade extends AbstractNDCFacade implements FlightPriceFacade
{
	...
	protected void setCurrencySessionCurrency(final FlightPriceRQ flightPriceRQ)
	{
		if (Objects.nonNull(flightPriceRQ.getParameters()) && Objects.nonNull(flightPriceRQ.getParameters().getCurrCodes())
				&& CollectionUtils.isNotEmpty(flightPriceRQ.getParameters().getCurrCodes().getCurrCode()))
		{
			getStoreSessionFacade().setCurrentCurrency(flightPriceRQ.getParameters().getCurrCodes().getCurrCode().get(0).getValue());
		}
	}
	...
	{
		final DataListType dataLists = new DataListType();
	...
	{
			flightPriceRQ.getTravelers().getTraveler().forEach(traveler -> {
				final AnonymousTravelerType anonymousTraveler = traveler.getAnonymousTraveler().get(0);
				anonymousTraveler.setObjectKey(anonymousTraveler.getPTC().getValue());
				flightPriceRS.getDataLists().getAnonymousTravelerList().getAnonymousTraveler().add(anonymousTraveler);
			});
	}
			...
			throws NDCOrderException
	{
		...
		final Optional<AnonymousTravelerType> anonymousTraveler = flightPriceRS.getDataLists().getAnonymousTravelerList()
				.getAnonymousTraveler().stream()
				.filter(anonymousTravelerType -> StringUtils.equals(anonymousTravelerType.getObjectKey(), ndcOfferItemId.getPtc()))
				.findFirst();

		if (!anonymousTraveler.isPresent())
		{
		...
		populatePriceDetails(requestedDate, ndcOfferItemId, anonymousTraveler.get(), transportOfferings);
	...
	protected void populatePriceDetails(final RequestedDate requestedDate, final NDCOfferItemId ndcOfferItemId,
			final AnonymousTravelerType anonymousTraveler, final Map<String, Map<String, TransportOfferingModel>> transportOfferings)
			throws NDCOrderException
	{
		...
		for (final NDCOfferItemIdBundle ndcOfferItemIdBundle : ndcOfferItemId.getBundleList())
		{
			...
			final double taxes = calculateTaxes(basePrice, product, anonymousTraveler, bundleTransportOfferings);
			...
			for (final ProductModel ancillaries : getAncillariesFromChildTemplate(bundleTemplate))
			{
				...
				final double ancillaryBasePrice = calculateBasePrice(ancillariesBundleTemplate, ancillaries, bundleTransportOfferings,
						ndcOfferItemId.getRouteCode());
				final double ancillaryTaxes = calculateTaxes(basePrice, ancillaries, anonymousTraveler, bundleTransportOfferings);
		...
		totalBasePrice = totalBasePrice * anonymousTraveler.getPTC().getQuantity().intValue();
	 ...
	 */
	protected double calculateTaxes(final double basePrice, final ProductModel product,
			final AnonymousTravelerType anonymousTraveler, final List<TransportOfferingModel> transportOfferings)
	{
		...
		setTaxSearchCriteriaInContext(transportLocationCodes, countryCodes,
				getNdcPassengerTypeService().getPassengerType(anonymousTraveler.getPTC().getValue()).getCode());
		...
		return taxValues.stream().mapToDouble(TaxValue::getValue).sum() * anonymousTraveler.getPTC().getQuantity().intValue();
	}
			...
			final String ptc)
	{
		...
		final TravelerInfoAssocType travelerInfoAssocType = new TravelerInfoAssocType();

		final Optional<AnonymousTravelerType> anonymousTraveler = flightPriceRS.getDataLists().getAnonymousTravelerList()
				.getAnonymousTraveler().stream()
				.filter(anonymousTravelerType -> StringUtils.equals(anonymousTravelerType.getObjectKey(), ptc)).findFirst();
		...
		travelerInfoAssocType.getTravelerReferences().add(anonymousTraveler.get());
		assocType.setAssociatedTraveler(travelerInfoAssocType);
