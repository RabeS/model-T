...
/**
 * unit test for Travel accommodation map handler.
 */
@UnitTest
@RunWith(MockitoJUnitRunner.class)
public class TravelAccommodationMapHandlerTest
{
	...
	@InjectMocks
	private final TravelAccommodationMapHandler travelAccommodationMapHandler = new TravelAccommodationMapHandler()
	{
		@Override
		protected void populateFromPrice(final OfferRequestData offerRequestData, final CabinData cabinData)
		{
			...
			cabinData.setPriceData(mockOfferRequestData.createPriceData());
		}
	};
	...
	public void accommodationMapItinerariesDoesNotExistTest()
	{
		...
		travelAccommodationMapHandler.handle(offerRequestData, offerResponseData);
	...
	public void populateSeatMap()
	{
		...
		travelAccommodationMapHandler.setConfiguredAccomNumberComparator(configuredAccomNumberComparator);
		Mockito
		...
		Mockito.when(travelCommercePriceFacade.createPriceData(Matchers.any(PriceDataType.class), Matchers.any(BigDecimal.class),
				Matchers.anyString())).thenReturn(testDataSetUp.createPriceData());
		Mockito.when(commerceStockService.getStockLevelQuantity(Matchers.any(ProductModel.class), Matchers.any(Collection.class)))
				.thenReturn(100L);
	...
	private class TestDataSetUp
	{
		...
		private ConfiguredAccommodationModel createConfiguredAccommodationModel(final boolean deck)
		{
			...
			{
				@Override
				public ConfiguredAccommodationType getType()
				{
					return ConfiguredAccommodationType.CABIN;
				}

				@Override
				public List<ConfiguredAccommodationModel> getConfiguredAccommodation()
				{
					return Stream.of(rowConfigModel).collect(Collectors.<ConfiguredAccommodationModel> toList());
				}
