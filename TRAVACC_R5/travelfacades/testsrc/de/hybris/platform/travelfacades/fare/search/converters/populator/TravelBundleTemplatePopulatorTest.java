...
/**
 * unit test for Travel bundle template populator.
 */
@UnitTest
...
public class TravelBundleTemplatePopulatorTest
{
	...
	public void populateTargetTest()
	{
		...
		Assert.assertEquals(1, travelBundleTemplate.getFareProducts().size());
	...
	private static class TestDataSetup
	{
		...
		private static List<BundleTemplateModel> createChildTemplates()
		{
			...
			childTemplate1.getProducts()
					.add(createProductModel(ProductType.ACCOMMODATION, "ACPECOSEAT1", "Economy Plus Seat"));
		...
		private static ProductModel createProductModel(final ProductType productType, final String code, final String name)
		{
			final ProductModel productModel = new ProductModel();
			productModel.setProductType(productType);
			productModel.setCode(code);
			productModel.setName(name, Locale.ENGLISH);
			populateProductData(productModel);
