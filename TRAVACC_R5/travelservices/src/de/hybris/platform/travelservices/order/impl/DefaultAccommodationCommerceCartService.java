		...
		implements AccommodationCommerceCartService
{
	...
	private OrderTotalPaidForEntryGroupCalculationStrategy orderTotalPaidForEntryGroupCalculationStrategy;
	...
	@Override
	public String getPaymentTypeForChangeDates(final AccommodationOrderEntryGroupModel accommodationOrderEntryGroup,
			final List<Integer> entryNumbers)
	{
		final CartModel cartModel = getCartService().getSessionCart();
		final BigDecimal totalPaidForGroup = getOrderTotalPaidForEntryGroupCalculationStrategy()
				.calculate(cartModel, accommodationOrderEntryGroup);

		final List<AbstractOrderEntryModel> entries = cartModel.getEntries().stream()
				.filter(entry -> entryNumbers.contains(entry.getEntryNumber())).collect(
						Collectors.toList());
		entries.forEach(entry-> entry.setEntryGroup(accommodationOrderEntryGroup));
		final BigDecimal entriesTotal = getTravelCartService().getEntriesTotal(entries);

		if (entriesTotal.compareTo(totalPaidForGroup) == 0)
		{
			return TravelservicesConstants.ORDER_AMOUNT_PAYABLE_STATUS_SAME;
		}

		return entriesTotal.compareTo(totalPaidForGroup) > 0 ? TravelservicesConstants.ORDER_AMOUNT_PAYABLE_STATUS_PAYABLE
	...
	/**
	 * @return the orderTotalPaidForEntryGroupCalculationStrategy
	 */
	protected OrderTotalPaidForEntryGroupCalculationStrategy getOrderTotalPaidForEntryGroupCalculationStrategy()
	{
		return orderTotalPaidForEntryGroupCalculationStrategy;
	}

	/**
	 * @param orderTotalPaidForEntryGroupCalculationStrategy
	 * 		the orderTotalPaidForEntryGroupCalculationStrategy to set
	 */
	@Required
	public void setOrderTotalPaidForEntryGroupCalculationStrategy(
			final OrderTotalPaidForEntryGroupCalculationStrategy orderTotalPaidForEntryGroupCalculationStrategy)
	{
		this.orderTotalPaidForEntryGroupCalculationStrategy = orderTotalPaidForEntryGroupCalculationStrategy;
	}
	...
	/**
	 *
	 * @param travelCartService the travelCartService to set
	 */
	@Required
