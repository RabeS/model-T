...
public class DefaultAutoAccommodationAllocationStrategy implements AutoAccommodationAllocationStrategy
{
	private static final Logger LOG = Logger.getLogger(DefaultAutoAccommodationAllocationStrategy.class);
	...
	protected void getTransportOfferingTravelAccommodationMap(final AbstractOrderModel abstractOrderModel,
			...
			final long numberOfSeatsToAllocate)
	{
		if (!vehicleInfo.isPresent())
		{
			return;
		...
		travellers.forEach(traveller -> {
					...
					traveller))
			{
				getFirstAvailableSeat(seatMapConfigurations, true, null, selectedAccommodations, transportOffering,
						travellerAccommodationMap, traveller);
			...
			final Map<TravellerModel, ConfiguredAccommodationModel> travellerAccommodationMap, final TravellerModel traveller)
	{
		...
		{
			final boolean isSeatFound = findAvailableSeat(isFirstAttempt, previousAccommodationType, selectedAccommodations,
					transportOffering, travellerAccommodationMap, traveller, configuredAccomModel);
			if (isSeatFound)
			{
				break;
			}
	...
	protected boolean findAvailableSeat(final boolean isFirstAttempt, final ConfiguredAccommodationType previousAccommodationType,
			final List<SelectedAccommodationModel> selectedAccommodations, final TransportOfferingModel transportOffering,
			final Map<TravellerModel, ConfiguredAccommodationModel> travellerAccommodationMap, final TravellerModel traveller,
			final ConfiguredAccommodationModel configuredAccomModel)
	{
				...
				AccommodationEnum.mapConfiguredAccommodationTypeToAccommodationEnum(configuredAccomModel.getType())))
		{
			...
			{
				getFirstAvailableSeat(configuredAccomModel.getConfiguredAccommodation(), false, configuredAccomModel.getType(),
						selectedAccommodations, transportOffering, travellerAccommodationMap, traveller);
			}
			else
			{
	...
	{
		final Long stock = getCommerceStockService().getStockLevelQuantity(seat.getProduct(),
				Collections.singleton(transportOffering));
