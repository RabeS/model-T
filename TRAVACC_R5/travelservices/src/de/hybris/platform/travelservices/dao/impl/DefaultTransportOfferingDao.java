...
public class DefaultTransportOfferingDao extends DefaultGenericDao<TransportOfferingModel> implements TransportOfferingDao
{
	...
	private static final String FIND_TRANSPORT_OFFERINGS = "SELECT {to." + TransportOfferingModel.PK + "} FROM {"
			+ TransportOfferingModel._TYPECODE + " AS to}";
			...
			throws AmbiguousIdentifierException, ModelNotFoundException
	{
		...
		{
			throw new ModelNotFoundException("No Transport Offering found for the given code: "+ code);
		}
	...
	@Override
	public SearchResult<TransportOfferingModel> findTransportOfferings(final int batchSize, final int offset)
	{
		final FlexibleSearchQuery flexibleSearchQuery = new FlexibleSearchQuery(FIND_TRANSPORT_OFFERINGS);
		flexibleSearchQuery.setCount(batchSize);
		flexibleSearchQuery.setStart(offset);
		flexibleSearchQuery.setNeedTotal(true);

		final SearchResult<TransportOfferingModel> searchResult = getFlexibleSearchService().search(flexibleSearchQuery);
		return searchResult;
	}
