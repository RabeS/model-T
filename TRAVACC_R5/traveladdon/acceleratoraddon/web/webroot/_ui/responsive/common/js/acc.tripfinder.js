ACC.tripfinder = {
	...
	tripFinderDestinationLocation: function () {
		...
		$(document).on("click", ".y_searchBtn", function () {
			//get data-id attribute of the clicked element
			var destinationLocationCode = $(this).data('destination-code');
			var destinationLocationName = $(this).data('destination-name');
	...
	bindGeoLocation: function () {
		...
		// if origin is in the URL, we can directly get the destination locations
			...
			var originJsonObject = {
				code: $(".y_tripFinderOriginLocationCode").val(),
				type: $(".y_tripFinderOriginLocationSuggestionType").val()
			};
			$.cookie(cookieName, originJsonObject, {expires: 10});
			ACC.tripfinder.showDestinationLocations();
		}
