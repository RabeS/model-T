...
public class DefaultNDCOrderEntryFacadeTest
{
	...
	public void testCreateOrderEntriesWithConversionException() throws CalculationException, NDCOrderException
	{
		...
		createPassengerListForConversionException(query);
		...
		Mockito
				.when(defaultNDCOrderEntryFacade
						.getTravellers(orderCreateRQ.getQuery().getDataLists().getPassengerList().getPassenger(), order.getCode()))
				.thenReturn(createTravellers());
	...
	private void createPassengerListForConversionException(final Query query)
	{
		final DataLists dataLists = new DataLists();
		final PassengerList passengerList = new PassengerList();
		final List<PassengerType> passengerTypes = passengerList.getPassenger();

		final Passengers passengers = new Passengers();
		final PassengerType passengers1 = new PassengerType();

		passengers1.setPTC("TEST_PTC_VALUE");
		passengers1.setPassengerID("TEST_PASSENGER_ID");


		passengerTypes.add(passengers1);
	...
	public List<TravellerModel> createTravellers()
	{
		final List<TravellerModel> travellers = (List<TravellerModel>) new TravellerModel();
