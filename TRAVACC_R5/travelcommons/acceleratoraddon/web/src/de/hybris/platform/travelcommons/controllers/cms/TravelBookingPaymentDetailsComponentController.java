...
/**
 * The type Travel booking payment details component controller.
 */
...
public class TravelBookingPaymentDetailsComponentController extends AbstractBookingDetailsComponentController
{
			...
			final AbstractBookingDetailsComponentModel component)
	{
		...
		if (isPackageInOrder)
		{
			...
			final RateData reservationTotals = getBookingFacade().getBookingTotals(bookingReference);
			model.addAttribute(TravelacceleratorstorefrontWebConstants.TOTAL, reservationTotals);
			model.addAttribute(TravelacceleratorstorefrontWebConstants.IS_AMENDED_CART, getBookingFacade().isAmendment(bookingReference));
		}
	}
}
