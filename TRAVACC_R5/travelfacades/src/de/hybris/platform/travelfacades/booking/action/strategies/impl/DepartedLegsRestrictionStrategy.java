...
/**
 * Strategy to evaluate the enabled property of the List<BookingActionData>. If all legs of the booking have at least one
 * TransportOffering in status DEPARTED, the enabled property is set to false, true otherwise
 */
public class DepartedLegsRestrictionStrategy implements BookingActionEnabledEvaluatorStrategy
{

	private static final String ALTERNATIVE_MESSAGE = "booking.action.departed.legs.alternative.message";
	...
	public void applyStrategy(final List<BookingActionData> bookingActionDataList, final ReservationData reservationData)
	{
		...
		for (final ReservationItemData reservationItem : reservationData.getReservationItems())
		{
			...
			enabled |= transportOfferings.stream().noneMatch(to -> getNotAllowedStatuses().stream().
					anyMatch(transportOfferingStatus -> StringUtils.equalsIgnoreCase(transportOfferingStatus.getCode(),
							to.getStatus())));
		}
