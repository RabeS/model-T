...
{
	private static final Logger LOG = Logger.getLogger(DefaultAccommodationOfferingFacade.class);
			...
			final AccommodationSearchRequestData accommodationRequestData, final RoomStayCandidateData roomStayCandidateData)
	{
		...
		if (accommodationRequestData.getCriterion().getPropertyType() != null)
		{
			filterTerms.put(TravelfacadesConstants.SOLR_FIELD_PROPERTY_TYPE, accommodationRequestData.getCriterion().getPropertyType());
		}
	...
	public PropertyData getPropertyData(final String accommodationOfferingCode)
	{
		...
		try
		{
			accommodationOfferingModel = getAccommodationOfferingService().getAccommodationOffering(accommodationOfferingCode);
		}
		catch (final ModelNotFoundException e)
		{
			LOG.debug("No accommodationOfferings found for code " + accommodationOfferingCode);
			return null;
		}
