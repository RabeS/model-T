...
public class DefaultTravelBundleTemplateDao extends DefaultBundleTemplateDao implements TravelBundleTemplateDao
{
	...
	private static final String FIND_BUNDLE_TEMPLATE_BY_CODE_QUERY = "SELECT {" + BundleTemplateModel.PK + "} FROM {"
			+ BundleTemplateModel._TYPECODE + "} where {" + BundleTemplateModel.ID + "}= ?uid and  {" + BundleTemplateModel.CATALOGVERSION + "} = ?catalogVersion ";
	...
	@Override
	public BundleTemplateModel findBundleTemplateByCodeAndCatalogVersion(final String bundleId, final CatalogVersionModel catalog) throws ModelNotFoundException,AmbiguousIdentifierException
	{
		validateParameterNotNullStandardMessage("bundleId", bundleId);
		final FlexibleSearchQuery query = new FlexibleSearchQuery(FIND_BUNDLE_TEMPLATE_BY_CODE_QUERY);
		query.addQueryParameter("uid", bundleId);
		query.addQueryParameter("catalogVersion", catalog);
