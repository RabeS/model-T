...
@UnitTest
public class TransportOfferingOriginLocationCityValueProviderTest extends PropertyFieldValueProviderTestBase
{
	...
	@Mock
	private TransportFacilityModel origin;
	...
	private static final String TEST_ORIGIN_LOCATION_CITY_PROP = "destinationLocationCity";

	private static final String TEST_ORIGIN_LOCATION_CITY_EN = "originLocationCity_en_string";

	private static final String TEST_ORIGIN_LOCATION_CITY_DE = "originLocationCity_de_string";

	private static final String TEST_LOCATION_OPTION = "ORIGIN";

	@Override
	protected String getPropertyName()
	{
		return TEST_ORIGIN_LOCATION_CITY_PROP;
	}
	 ...
	 */
	@Test
	public void testReturnDestinationLocationWhenTransportOfferingHasLocationCity() throws FieldValueProviderException
	{

		Mockito.when(transportOffering.getTravelSector()).thenReturn(sector);
		Mockito.when(sector.getOrigin()).thenReturn(origin);
		Mockito.when(origin.getLocation()).thenReturn(location);

		Mockito.when(location.getLocationType()).thenReturn(LocationType.CITY);
		Mockito.when(location.getName(Matchers.any(Locale.class))).thenReturn("PARIS");
		...
		Mockito.when(fieldNameProvider.getFieldNames(Matchers.<IndexedProperty> any(), Mockito.eq(TEST_EN_LANG_CODE)))
				.thenReturn(singletonList(TEST_ORIGIN_LOCATION_CITY_EN));
		Mockito.when(fieldNameProvider.getFieldNames(Matchers.<IndexedProperty> any(), Mockito.eq(TEST_DE_LANG_CODE)))
				.thenReturn(singletonList(TEST_ORIGIN_LOCATION_CITY_DE));
		...
		for (final FieldValue val : result)
		{
			...
			Assert.assertTrue(val.getFieldName().equalsIgnoreCase(TEST_ORIGIN_LOCATION_CITY_EN)
					|| val.getFieldName().equalsIgnoreCase(TEST_ORIGIN_LOCATION_CITY_DE));
		}
	}
	...
	@Test
	public void testShouldReturnEmptyValueWhenTransportOfferingOriginLocationIsNotCity() throws FieldValueProviderException
	{
	 ...
	 */
	@Test
	public void testShouldReturnValueWhenCityIsNotAtFirstLevelInOriginTransportFacility() throws FieldValueProviderException
	{
