ACC.ancillaryGroup = {
		...
		$(".y_applyOffer").on('click', function() {
			...
			if(!$.isEmptyObject(filteredAddToCartForms)) {
				...
				ACC.ancillaryGroup.performAddToCartForGroup(filteredAddToCartForms,function(addToCartResult) {
					$("#y_processingModal").dialog('close');
				});
			}
		});
	},
	...
	bindSelectNewAncillaryProduct : function(target) {
		
		target.on('change', function() {
			ACC.ancillaryGroup.bindAncillaryProduct(target);
		});
	},
	
	bindAncillaryProduct  : function(target) {
			var uniqueId=target.attr("data-offertabid")+"_"+target.attr("data-itemindex");
			...
			ACC.ancillaryGroup.enableOptionsBasedOnPassengerTypes(target);
			ACC.ancillaryGroup.refreshAncillaryDeleteBlock(target);
			ACC.ancillaryGroup.refreshVisibilityAddAnotherItemLink(target);
			ACC.ancillaryGroup.refreshSelectAncillaryOptions(target.attr("data-offertabid"));
			...
			ACC.ancillaryGroup.disableOptionsBasedOnPassengerTypes(target);
	},
	
	bindSelectAncillaryProduct : function() {
		
		$(".y_selectAncillaryProduct").on('change', function() {
			ACC.ancillaryGroup.bindAncillaryProduct($(this));
		});
	},
	...
	refreshAncillaryDeleteBlock: function(selectAncillaryProduct) {
		...
		if(selectAncillaryProduct.val() !== null && selectAncillaryProduct.val().trim() || selectItemNos > 1) {
			removeLink.show();				
		} else {
			removeLink.hide();
		}
	...
	refreshVisibilityAddAnotherItemLink: function(selectItemObj) {
		...
		if (ACC.ancillaryGroup.hasEmptyRow(offerTabId, selectItemObj)) {
	...
	hasEmptyRow: function(offerTabId, selectItemObj) {
		var selectItemNosVisible=$(".y_selectAncillaryProduct[data-offertabid='" + offerTabId + "']:visible").length;
		var offerTabId=selectItemObj.attr("data-offertabid");
		var selectItemObjOptionLength=selectItemObj.find("option").length-1;
		
		var atleastOneEmpty = false;
		$(".y_selectAncillaryProduct[data-offertabid='" + offerTabId + "']:visible").each(function(index, selectAncillaryProduct ) {
			var selectedValue=selectAncillaryProduct.value;
			if(selectedValue=="") {
				atleastOneEmpty = true; 
			}
		});
		
		if (atleastOneEmpty || selectItemNosVisible == selectItemObjOptionLength) {
			return true;
		} else {
			return false;
		}
	...
	bindAncillaryRemoveLink: function() {
		$(".y_ancillaryRemoveItem").on('click', function() {
			...
			var offerType=$(this).attr("data-offertype");
			var ancillaryRemoveLinkLength=$(".y_selectAncillaryProduct[data-offertabid='"+offerTabId+"']:visible").length;
			...
			if(divToRemove.length && ancillaryRemoveLinkLength) {
				if(ancillaryRemoveLinkLength==1) {
					...
					$(this).hide();
				}else if(ancillaryRemoveLinkLength == 2){
					divToRemove.remove();
			...
			if (ACC.ancillaryGroup.hasEmptyRow(offerTabId, $(this))) {
				$(".y_ancillaryAddAnotherItem#y_addAnotherItem_"+offerTabId).fadeOut('slow');
			} else {
				$(".y_ancillaryAddAnotherItem#y_addAnotherItem_"+offerTabId).fadeIn('slow');
			}
	...
	bindAddAnotherAncillaryItem: function() {
		$(".y_ancillaryAddAnotherItem").on('click', function() {
			...
			ACC.ancillaryGroup.bindTravellersSelect();
			ACC.ancillaryGroup.bindSelectNewAncillaryProduct(newDiv.find(".y_selectAncillaryProduct")); 
	...
	disableOptionsBasedOnPassengerTypes: function(target) {
		var uniqueId = target.attr("data-offertabid")+"_"+target.attr("data-itemindex");
		var list = target.parent().parent().find(".optWrapper li");
		$(".multi-select#y_travellersSelecter_"+uniqueId).find("option").each(function(index, option) {
			if($(option).is(':disabled')){
				list.eq(index).addClass('disabled');
			}
			else{
				list.eq(index).removeClass('disabled');
				list.eq(index).removeClass('selected');
				list.eq(index).prop('checked', false);
			}
		});
	},
	
	enableOptionsBasedOnPassengerTypes: function(target) {
		var uniqueId = target.attr("data-offertabid")+"_"+target.attr("data-itemindex");
		var travellersSelecterId="y_travellersSelecter_"+uniqueId;
		ACC.ancillaryGroup.sumoUnSelectAll($("#"+travellersSelecterId));
	}
