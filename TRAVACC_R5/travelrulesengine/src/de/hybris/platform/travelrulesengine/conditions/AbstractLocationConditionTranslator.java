...
public abstract class AbstractLocationConditionTranslator implements RuleConditionTranslator
{
			...
			final RuleConditionDefinitionData ruleConditionDefinitionData, final String contextRaoVariable)
	{
		...
		createGroupLocationOrCondition(inclMarketLocations, MARKET_LOCATIONS_ATTRIBUTE, contextRaoVariable, irLocationCondition,
				RuleIrAttributeOperator.CONTAINS);

		// EXCLUDED MARKET LOCATIONS
		createGroupLocationAndCondition(excMarketLocations, MARKET_LOCATIONS_ATTRIBUTE, contextRaoVariable, irLocationCondition,
	 ...
	 * 		the ir location condition
	 */
	protected void createAddressCondition(final String attribute, final String raoVariable,
			final RuleIrGroupCondition irLocationCondition)
	{
		final RuleIrGroupCondition irAddressLocationCondition = new RuleIrGroupCondition();
		irAddressLocationCondition.setOperator(RuleIrGroupOperator.AND);
		irAddressLocationCondition.setChildren(new ArrayList<>());

		final RuleIrAttributeCondition ruleIrAttributeCondition = new RuleIrAttributeCondition();
		ruleIrAttributeCondition.setVariable(raoVariable);
		ruleIrAttributeCondition.setAttribute(attribute);
		ruleIrAttributeCondition.setOperator(RuleIrAttributeOperator.NOT_EQUAL);
		ruleIrAttributeCondition.setValue(null);

		irAddressLocationCondition.getChildren().add(ruleIrAttributeCondition);

		irLocationCondition.getChildren().add(irAddressLocationCondition);
	}
}
