...
/**
 * Resovles the tax value of the RoomProduct(s) for given MarketingRatePlanInfo
 */
public class RoomRateTaxValueResolver extends AbstractPriceValueResolver
{
	@Override
			...
			final Qualifier qualifier, final Date documentDate)
	{
		...
		ratePlanConfigs.forEach(ratePlanConfigModel -> {
			...
			if (roomRateProduct != null)
			{
				...
				if (roomRateTaxValue != null)
				{
					ratePlanConfigPrice = roomRateTaxValue * ratePlanConfigModel.getQuantity();
				}
