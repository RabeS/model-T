...
public abstract class NDCAbstractOfferRSPopulator
{
	 ...
	 */
	protected void populatePriceData(final AnonymousTravelerList travelerList, final List<ServicePriceType> prices,
			final OfferPricingInfoData offerPricingInfo)
	{
		final Map<BigDecimal, List<AnonymousTravelerType>> map = createTravelerTypePriceMap(travelerList, offerPricingInfo);
	...
	/**
	 * This method creates the map having offer value as key and associated {@link AnonymousTravelerType}'s list as value.
	 *
	 * @param travelerList
	 * 		the traveler list
	 * @param offerPricingInfo
	 ...
	 * @return the map
	 */
	protected Map<BigDecimal, List<AnonymousTravelerType>> createTravelerTypePriceMap(final AnonymousTravelerList travelerList,
			final OfferPricingInfoData offerPricingInfo)
	{
		final Map<BigDecimal, List<AnonymousTravelerType>> map = new HashMap<>();
		final List<AnonymousTravelerType> passengerTypes = new ArrayList<>();
		offerPricingInfo.getTravellerBreakdowns().forEach(travellerBreakdownData -> {
			...
			final AnonymousTravelerType passengerRef = getPassengerRefernceByCode(travelerList, pid.getPassengerType().getCode());
	...
	/**
	 * This method returns the reference of {@link AnonymousTravelerType} for given code.
	 *
	 ...
	 */
	protected AnonymousTravelerType getPassengerRefernceByCode(final AnonymousTravelerList anonymousTravelers, final String code)
	{
		final Optional<AnonymousTravelerType> optional = anonymousTravelers.getAnonymousTraveler().stream()
				.filter(anonymousTraveler -> anonymousTraveler.getObjectKey().equalsIgnoreCase(code)).findFirst();
	 ...
	 * @return the boolean
	 */
	protected boolean isPassengerRefExist(final AnonymousTravelerType passengerRef,
			final Map<BigDecimal, List<AnonymousTravelerType>> map, final BigDecimal offerValue)
	{
		if (Objects.nonNull(passengerRef))
		{
			return CollectionUtils.isNotEmpty(map.get(offerValue)) ? map.get(offerValue).stream().anyMatch(
					anonymousTraveler -> StringUtils.equalsIgnoreCase(anonymousTraveler.getObjectKey(), passengerRef.getObjectKey()))
					: Boolean.FALSE;
		}
	...
	protected Settlement getSettlement()
	{
		...
		final CodesetType codesetType = new CodesetType();
		codesetType.setCode(NdcfacadesConstants.SETTLEMENT_METHOD);
		codesetType.setDefinition(NdcfacadesConstants.SETTLEMENT_DEF);
		settlement.setMethod(codesetType);
		return settlement;
	}
			...
			final ProductData productData, final OriginDestinationOfferInfoData offerInfoData)
	{
		...
		if (Objects.isNull(productData) || CollectionUtils.isNotEmpty(productData.getCategories()))
		{
				...
				&& !isOriginDestinationExist(flight, offerInfoData.getTravelRouteCode()))
		{
			...
			final JAXBElement<List<Object>> element = factory.createOriginDestinationReferences(
					Collections.singletonList(originDestination));
	...
	/**
	 * This method populates applicable {@link SegmentReferences}, if it is not already exist, to {@link ServiceAssocType.Flight}
	 *
	 * @param segmentKeys
	 * 		the segment keys
	...
	protected ListOfFlightSegmentType getSegmentForKey(final String segmentKey, final List<Object> value)
	{
		final Optional<Object> opt = value.stream()
				...
				.findFirst();
		return (ListOfFlightSegmentType) opt.orElse(null);
