...
public abstract class AbstractPaymentTransactionEntryCreationStrategy
{
	...
	protected BigDecimal getEntriesAmount(final List<AbstractOrderEntryModel> entries)
	{
		final List<AbstractOrderEntryModel> validEntriesToCalculate = entries.stream()
				...
				.collect(Collectors.toList());
		final BigDecimal entriesSubtotal = CollectionUtils.isEmpty(validEntriesToCalculate) ? BigDecimal.ZERO
				: BigDecimal
		...
		return getEntriesTotalWithDiscounts(entriesSubtotal, entries);
	}

	protected BigDecimal getEntriesTotalWithDiscounts(
			final BigDecimal paymentTotal, final List<AbstractOrderEntryModel> entries)
	{
		...
		final AbstractOrderEntryModel sampleEntry = entries.get(0);
		final OrderEntryType entryType = sampleEntry.getType();
		final AbstractOrderModel abstractOrder = sampleEntry.getOrder();
		...
		return paymentTotal.subtract(discount);
	}
	...
	/**
	 * @return the getGlobalDiscountCalculationStrategyMap
	 */
	 ...
	 * 		the getGlobalDiscountCalculationStrategyMap to set
	 */
	@Required
