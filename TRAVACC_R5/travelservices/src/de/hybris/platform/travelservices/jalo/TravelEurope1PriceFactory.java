...
public class TravelEurope1PriceFactory extends Europe1PriceFactory implements TravelPriceFactory
{
	...
	/**
	 * Runs the query to retrieve prices rows for product and travel search criteria. If there are no price rows returned, the
	 * default query to fetch the price rows is being invoked. Please note that this method may return more rows than allowed so
	 * make sure they're filtered afterwards!
	 *
	 * @param ctx
	 * @param product
	 ...
	 * @throws JaloPriceFactoryException
	 * @deprecated since version 5.0 after deprecation of the equivalent superclass method
	 */
	@Deprecated
	@Override
	...
	/**
	 * Runs the query to retrieve prices rows for product and travel search criteria. If there are no price rows returned, the
	 * default query to fetch the price rows is being invoked. This method overrides the superclass method introduced with Hybris
	 * Commerce Platform 6.7, keeping the previous travel accelerator functionality
	 *
	 * @param ctx
	 * @param product
	 * 		the product to get prices for
	...
	/**
	 * Method overridden to set order currency in session context. This currency will be used when filtering tax rows based on
	 * currency. Please note that hybris OOTB code for taxes doesn't handle filtering tax rows based on order currency (price rows
	 * implementation handles this though).
	 *
	 * @param entry
