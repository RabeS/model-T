...
import de.hybris.platform.travelservices.enums.CategoryMapping;
...
public class DefaultTravelCommercePriceService extends DefaultCommercePriceService implements TravelCommercePriceService
{
	...
	private TravelPDTRowDao travelPDTRowDao;
			...
			final boolean isMultiSectorRoute)
	{
		...
		if (!(productModel instanceof FareProductModel || ProductType.FARE_PRODUCT.equals(productModel.getProductType())))
		{
	...
	{
		final CategoryModel categoryModel = productModel.getSupercategories().stream().findFirst().orElseGet(null);

		final Optional<CategoryMapping> categoryMapping =
				Objects.nonNull(categoryModel) && OfferGroupModel.class.isInstance(categoryModel) ?
						Optional.of(OfferGroupModel.class.cast(categoryModel)).map(OfferGroupModel::getCategoryMapping)
						: Optional.empty();
		final String priceLevelMapping = categoryMapping.isPresent() ? categoryMapping.get().getCode() :
				getOfferGroupToOriginDestinationMapping().getOrDefault(
						Objects.nonNull(categoryModel) ? categoryModel.getCode() : StringUtils.EMPTY,
						getOfferGroupToOriginDestinationMapping().getOrDefault(
	...
	@Override
	public Boolean isNetPricedProduct(final ProductModel product)
	{
		PriceRowModel firstAvailablePriceForProduct = null;
		try
		{
			firstAvailablePriceForProduct = getTravelPDTRowDao().findFirstAvailablePriceForProduct(product);
		}
		catch (final ModelNotFoundException mnfEx)
		{
			LOG.error(String.format("No price retrieved for the product with code %s", product.getCode()), mnfEx);
		}
		return Objects.nonNull(firstAvailablePriceForProduct) ? firstAvailablePriceForProduct.getNet() : null;
	}
	...
	/**
	 * @return the travelPDTRowDao
	 */
	protected TravelPDTRowDao getTravelPDTRowDao()
	{
		return travelPDTRowDao;
	}

	/**
	 * @param travelPDTRowDao
	 * 		the travelPDTRowDao to set
	 */
	@Required
	public void setTravelPDTRowDao(final TravelPDTRowDao travelPDTRowDao)
	{
		this.travelPDTRowDao = travelPDTRowDao;
	}
