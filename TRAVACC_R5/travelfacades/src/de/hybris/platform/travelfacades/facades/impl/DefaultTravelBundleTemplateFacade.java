...
public class DefaultTravelBundleTemplateFacade implements TravelBundleTemplateFacade
{
			...
			final ItineraryPricingInfoData availableUpgradeItineraryPricingInfoData)
	{
		...
		availableUpgradeItineraryPricingInfoData.getTotalFare()
				.setPriceDifference(getTravelCommercePriceFacade().createPriceData(diffAmountForUpgrade, 2));
	}
	...
	@Override
	public TravelBundleTemplateData getBundleTemplateByCodeAndCatalogVersion(final String bundleTemplateId, final String catalogId) throws ModelNotFoundException,AmbiguousIdentifierException
	{
		return getTravelBundleDataTemplateConverter()
				.convert(getTravelBundleTemplateService().getBundleTemplateByCodeAndCatalogVersion(bundleTemplateId, catalogId));
	}
