		...
		<form:form id="${fn:escapeXml(formId)}" method="${fn:escapeXml(method)}" autocomplete="off" commandName="transportOfferingStatusForm" action="${fn:escapeXml(action)}" class="col-xs-12 form-background">
			<fieldset>
				<legend class="primary-legend">
					<a class="panel-heading panel-header-link collapsable collapsed y_flightStatusPanelHeaderLink" role="button" data-toggle="collapse" data-target="#flight-status" aria-expanded="false" aria-controls="manage-booking"><spring:theme code="status.search.title" text="Flight Status" /></a>
				</legend>
