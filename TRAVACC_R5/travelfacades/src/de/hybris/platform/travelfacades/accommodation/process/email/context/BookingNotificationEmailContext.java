...
public class BookingNotificationEmailContext extends AbstractBookingEmailContext
{
	...
	{
		final Double totalPaidAmount = accommodationReservationData.getTotalRate().getActualRate().getValue().doubleValue()
				- accommodationReservationData.getTotalToPay().getValue().doubleValue();
		return getTravelCommercePriceFacade()
				.createPriceData(PriceDataType.BUY, BigDecimal.valueOf(totalPaidAmount), order.getCurrency().getIsocode())
				.getFormattedValue();
	}
