...
/**
 * Handler to populate seat map options for a transport vehicle retrieved from DB based on mapping between the vehicle config and
...
public class TravelAccommodationMapHandler implements AncillarySearchHandler
{
	 ...
	 * itinerary and then for each transport offering in origindestinationoption get the transport vehicle to retrieve seat map for
	 * that vehicle 3) Using vehicle info code and transport offering, route and sector, accommodation map is retrieved 4) Finally
	 * all configured accommodation(decks, cabins, rows, columns and seats) are retrieved and populated in the
	...
	/**
	 * Create the status parameters to find selected accommodations that belong to an order, for making accommodations already
	 * selected and already added to order(not in cancelled state), unavailable
	 *
	 * @param offerRequestData
	 * 		the offer request data
	...
	/**
	 * This method does following : 1)Checks if seatMapConfigurations i.e. Deck(s) exist, if not then it returns. 2)Else : a)it
	 * instantiates a seatmapdata for current transport offering(within the loop) b)instantiates and associate a
	 * seatmapdetail(containing a list of cabins) to seatmapdata c)adds seatmapdata to the list of seatmaps belonging to
	 * SeatMapResponseData. 3)Creates a modifiable list of current list of configured accommodations(i.e. deck) so as to sort them
	 * based on deck no. using configuredAccomNumberComparator, this can be used to position more than one decks. 4)Start looping
	 * through current list of configured accommodations i.e., and for first Deck, calls "populateAccommodationMapDetailData" to
	 * populate child configured accommodation(cabin, row, columns etc.) belonging to each deck.
	 *
	 * @param offerRequestData
	 * 		the offer request data
	 * @param seatMapResonseData
	 * 		the seat map resonse data
	 * @param seatMapConfigurations
	 * 		the seat map configurations
	 * @param transportOfferingData
	 * 		the transport offering data
	...
	/**
	 * This method does following : 1) Checks if current configured accommodation(deck) contains cabin, if Not it returns 2) Else :
	 * a)Create a modifiable list of cabins, to sort them based on number using configuredAccomNumberComparator b)For each
	 * configured accommodation(cabin) get all the rows and create a modifiable list of rows to sort them based on number using
	 * configuredAccomNumberComparator. 3)Gets the row number of first row from sorted rows and sets that as starting row num of
	 * current cabin, gets last row num of the sorted rows and sets that as ending row num of the cabin(these are used to draw
	 * total rows of a cabin on FE) 4)Check and set upper deck indicator, associates cabin class data to determine cabin class on
	 * FE and set column headers for all accommodation columns belonging to current cabin. 5)Finally it calls "populateRowInfo" to
	 * populate rows, columns and accommodation info of all accommodations within current cabin.
	 *
	 * @param offerRequestData
	 * 		the offer request data
	 * @param seatMapDetail
	...
	/**
	 * This method gets all accommodations belonging to all row/column of a cabin, and sort them in ascending order by price, to
	 * get the lowest accommodation fare of the cabin, this is done to display "From LowestPrice" for a cabin on FE.
	 *
	 * @param offerRequestData
	 * 		the offer request data
	 * @param cabinData
	...
	/**
	 * This method does following : 1)Checks if the rows(belonging to a cabin) and passed to it as parameter is empty, it returns
	 * if the theres no rows. 2)Else : a)Loop through all the rows and for each row create a RowInfoData and calls
	 * "populateSeatInfo" to associate accommodation info of all accommodations belonging to current row. b)adds this rowinfodata
	 * to list of rowinfos belonging to a cabin
	 *
	 * @param offerRequestData
	 * 		the offer request data
	 * @param cabinData
	...
	/**
	 * This method does following : 1)For the row passed to it as parameter it gets all columns and sort them using
	 * configuredAccomNumberComparator to have column sorted based on number. 2)It checks if modifiable list of column is empty,
	 * returns if it is empty, Else : a)Loop through columns of the row, and for each column, gets all accommodations belonging to
	 * the column. b)Calls "createSeat" method to create the accommodation info.
	 *
	 * @param offerRequestData
	 * 		the offer request data
	 * @param row
			...
			final Map<ProductModel, Long> productStockMap)
	{
		...
		for (final ConfiguredAccommodationModel column : modifiableColumns)
		{
			...
			{
				LOG.error("Seat Map configured accommodation mismatch, expected COLUMN , received : " + column.getType());
				continue;
			}
			++colNum;
	...
	/**
	 * This method does following : 1)Create a seatinfodata object for an accommodation, and adds it to list of seatinfos passed to
	 * it as parameter 2)Sets the column no. to which this accommodation belong. 3)Check if seat have proximity items(WC, GALLEY
	 * etc.) associated with it, if yes then creates proximity item data and associate it with seat. 4)Calls "populateSeatFare" to
	 * set fare for each seat 5)Calls "populateSeatAvailability" to set the availabilty of seat.
	 *
	 * @param offerRequestData
	 * 		the offer request data
	 * @param seatInfo
	...
	/**
	 * This method does following : 1)For the seat passed to it as parameter, it gets product associated with the seat. 2)If the
	 * product is not null, it checks if productStockMap passed as param has entry of the product as key, if found, it gets stock
	 * of that product as value against the key. 3)If product is not found, it checks stock of product in DB, and sets it as
	 * key/value pair in productStockMap(to avoid hitting DB to check stock of same product, in case theres same product associated
	 * with another seat) 4)If stock is 0, it calls "createSeatAvailabilityData" to create SeatAvailabilityData for that seat with
	 * availability indicator = N 5)If stock is not 0, it checks for selected accommodation for this seat in the
	 * selectedAccommodations(list of all accommodations belonging to this transport offering already added to order) passed to it
	 * as param. 6)If selectedAccommodation is found for this seat, it calls "createSeatAvailabilityData" to create
			...
			final TransportOfferingModel transportOffering, final Map<ProductModel, Long> productStockMap)
	{
		...
		if (product != null)
		{
			...
			else
			{
				try
				{
					stock = getCommerceStockService().getStockLevelQuantity(product, Collections.singleton(transportOffering));
			...
			{
				//null stock is returned from FORCE_IN_STOCK stock level
				productStockMap.put(product, (long) -1);
				return;
			}
	...
	/**
	 * create a seat availabilty data for a seat and set its availability indicator to D (disable) to represent non-availability of
	 * that seat.
	 *
	 * @param offerRequestData
	 * 		the offer request data
	...
	/**
	 * create a seat availabilty data for a seat and set its availability indicator to N to represent non-availability of that
	 * seat.
	 *
	 * @param offerRequestData
	 * 		the offer request data
	...
	/**
	 * This method does following : For seat passed to it as parameter : 1)Gets associated product. 2)Gets price information for
	 * associated product and transport offering 3)If not found, look for price information for associated product and travel
	 * sector 4)If not found, look for price information for associated product and travel route 5)If not found, look for price
	 * information for associated product 6)If still not found then do not associate price with the seat else, create TotalFareData
	 * based on price information and associate it with seatInfoData(representing current seat) passed as param
	 *
	 * @param offerRequestData
	 * 		the offer request data
	 * @param seatInfoData
	...
	/**
	 * Method takes a PriceInformation object and performs a null check. If the object a is null then null will be returned
	 * otherwise the PriceDataFactory is invoked and a new PriceData object is created and returned using the PriceValue on the
