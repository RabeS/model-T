ACC.guestdetails = {
    	...
    	$.when(ACC.services.addExtraToCartAjax()).then(
    			function(response) {
    	            ...
    	            if (jsonData.valid) {
    					...
    					$("#y_addExtraToCartErrorModal").dialog('open');
    	            }
            ...
            $.when(ACC.services.removeRoomAjax(href)).then(
	            function(data) {
                	if(data.SUCCESS) {
                        ...
                        $("#y_removeRoomResultMessageModal").dialog('open');
	...
	bindEnableConfirmButton : function(){
    	$(document).ready(function() {
            var $form = $("#y_guestDetailsForm");
            $form.find('.y_guestDetailsContinueButton').prop('disabled', false);
        });
    }
};
