			...
			<div class="${ showComponent ? '' : 'panel-modify collapse'} ">
				<div id="accommodation-finder-panel" class="fieldset-inner-wrapper">
					...
					<%-- Begin No of Accommodations input field section --%>
						...
						<div class="col-xs-12 col-sm-6 age-info">
	                        <c:set var="infoText">
				...
				<div class="row input-row less-margin accommodation-type">
					<%-- Begin Property Type input field section --%>
                    <div class="col-xs-12 col-sm-6">
                        <form:select class="form-control y_accommodationFinderLocationPropertyType"  id="y_accommodationFinderLocationPropertyType" path="accommodationPropertyType">
                            <form:option value="-1" disabled="true" selected="selected">
                                <spring:theme code="text.cms.accommodationfinder.propertytype" text="Property Types"/>
                            </form:option>
                            <form:option value="">
                                <spring:theme code="text.cms.accommodationfinder.propertytype.nopreference" text="No Preference"/>
                            </form:option>
							<c:forEach var="entry" items="${accommodationPropertyType}">
								<form:option value="${fn:trim(entry.code)}" htmlEscape="true">
									${fn:escapeXml(entry.name)}
								</form:option>
							</c:forEach>
						</form:select>
                    </div>
                    <%-- End Property Type input field section --%>
				</div>
