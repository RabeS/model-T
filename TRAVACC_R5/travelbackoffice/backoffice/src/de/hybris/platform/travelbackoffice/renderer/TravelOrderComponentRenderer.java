...
/**
 * The Renderer class is used to render the Order Entries inside the on demand price section under the Position and
 * Prices Tab of the Order
 */
public class TravelOrderComponentRenderer implements WidgetComponentRenderer<Component, CustomSection, Object>
{
	private final TravelOrderOnDemandPricesHelper orderOnDemandPricesHelper;

	public TravelOrderComponentRenderer(final TravelOrderOnDemandPricesHelper orderOnDemandPricesHelper)
	{
			...
			final WidgetInstanceManager widgetInstanceManager)
	{
		...
		final OrderModel order = (OrderModel) model.getValue("currentObject", (Class) OrderModel.class);
		final TravelOrderInfo orderInfo = this.orderOnDemandPricesHelper.estimateOrderInfo(order);
		final Vlayout vlayout = new Vlayout();
		vlayout.appendChild(new Label(Labels.getLabel("hmc.msg.orderentries")));
		vlayout.appendChild(this.buildOrderEntryGrid(orderInfo));
		vlayout.appendChild(new Label(Labels.getLabel("hmc.msg.order")));
		vlayout.appendChild(this.buildOrderGrid(orderInfo));
		parent.appendChild(vlayout);
	}

	private Grid buildOrderEntryGrid(final TravelOrderInfo orderInfo)
	{
		...
		rows.setParent(grid);
		for (final TravelOrderEntryInfo orderEntryInfo : orderInfo.getTravelOrderEntryList())
		{
	...
	private Grid buildOrderGrid(final OrderInfo orderInfo)
	{
		final Grid grid = new Grid();
		grid.setWidth("650px");
		final Columns columns = new Columns();
		columns.setParent(grid);
		Column col = new Column(Labels.getLabel("hmc.msg.orderprice"));
		col.setWidth("250px");
		col.setParent(columns);
		col = new Column(Labels.getLabel("hmc.msg.ordertotaltax"));
		col.setWidth("150px");
		col.setParent(columns);
		col = new Column(Labels.getLabel("hmc.msg.orderpricetotal"));
		col.setWidth("250px");
		col.setParent(columns);
		final Rows rows = new Rows();
		rows.setParent(grid);
		final Row row = new Row();
		rows.appendChild(row);
		row.appendChild(new Label(String.valueOf(orderInfo.getTotalPrice())));
		row.appendChild(new Label(String.valueOf(orderInfo.getTotalTax())));
		row.appendChild(new Label(String.valueOf(orderInfo.getTotalWithTax())));
		return grid;
	}
}
