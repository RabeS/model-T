...
public class DefaultTravelBundleTemplateService extends DefaultBundleTemplateService implements TravelBundleTemplateService
{
	...
	public String getBundleTemplateIdFromOrder(final AbstractOrderModel abstractOrder, final int originDestinationRefNumber)
	{
		...
		for (final AbstractOrderEntryModel entry : abstractOrder.getEntries())
		{
			if ((entry.getProduct() instanceof FareProductModel
					|| Objects.equals(ProductType.FARE_PRODUCT, entry.getProduct().getProductType()))
	...
	@Override
	public BundleTemplateModel getBundleTemplateByCodeAndCatalogVersion(final String bundleId, final String catalogId) throws ModelNotFoundException,AmbiguousIdentifierException
	{
		return getTravelBundleTemplateDao().findBundleTemplateByCodeAndCatalogVersion(bundleId,
				getCatalogVersionService().getCatalogVersion(catalogId, DEFAULT_CATALOG_VERSION));
	}
