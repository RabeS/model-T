...
public class AccommodationCartEntryValidationStrategy extends AbstractCartEntryValidationStrategy
{
	...
	protected long getCartLevel(final CartEntryModel cartEntryModel, final CartModel cartModel)
	{
		...
		if (CollectionUtils.isNotEmpty(newEntries))
		{
			...
			{
				cartLevel = Long.sum(cartLevel, CollectionUtils.size(newEntries));
			}
			else if (product instanceof ExtraProductModel)
			{
				final long quantity = newEntries.stream().mapToLong(entry -> {
					final AccommodationOrderEntryGroupModel entryGroup = (AccommodationOrderEntryGroupModel) entry.getEntryGroup();
					return entry.getQuantity() / TravelDateUtils
							.getDaysBetweenDates(entryGroup.getStartingDate(), entryGroup.getEndingDate());
				}).sum();
				cartLevel = Long.sum(cartLevel, quantity);
			}
			else
			{
